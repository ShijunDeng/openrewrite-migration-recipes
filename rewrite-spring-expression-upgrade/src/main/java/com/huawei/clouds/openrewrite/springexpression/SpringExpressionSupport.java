...
final class SpringExpressionSupport {
    static final String GROUP = "org.springframework";
    static final String ARTIFACT = "spring-expression";
    ...
    private static final Pattern VERSION_NUMBERS = Pattern.compile("^(\\d+(?:[.]\\d+)*).*");
    ...
    private SpringExpressionSupport() {
    }
    ...
    static boolean hasVariant(J.MethodInvocation invocation) {
        return invocation.getArguments().stream().anyMatch(argument ->
                argument instanceof J.Assignment assignment &&
                        MAP_VARIANT_KEYS.contains(assignmentKey(assignment)) ||
    ...
    static int compareVersions(String left, String right) {
        int[] actual = numbers(left);
        int[] expected = numbers(right);
        if (actual == null || expected == null) return 0;
        int length = Math.max(actual.length, expected.length);
        for (int i = 0; i < length; i++) {
            int actualPart = i < actual.length ? actual[i] : 0;
            int expectedPart = i < expected.length ? expected[i] : 0;
            if (actualPart != expectedPart) return Integer.compare(actualPart, expectedPart);
        }
        return 0;
    }
    ...
    static int[] numbers(String version) {
        ...
        Matcher matcher = VERSION_NUMBERS.matcher(version);
        if (!matcher.matches()) return null;
        String[] parts = matcher.group(1).split("[.]");
        int[] numbers = new int[Math.max(3, parts.length)];
