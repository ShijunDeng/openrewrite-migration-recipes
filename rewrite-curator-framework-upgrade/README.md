# Apache Curator Framework 升级到 5.7.1

本模块对应表格坐标 `org.apache.curator:curator-framework`，严格处理表格列出的 `2.7.1`、`5.2.0`、`5.3.0`、`5.4.0` → `5.7.1`，不对表格外版本做范围推测。`2.7.1` 路径跨越约十年和三个大版本。README 是不兼容点 spec；真正的迁移入口会组合依赖升级、确定性 Java 改写和需要业务判断位置的精确标记。
...
推荐的 `MigrateCuratorFrameworkTo5_7_1` 按顺序执行版本升级、兼容性审计和确定性源码改写。SearchResult 标记会带上可操作原因，例如 Java 中的 `/*~~(RetryLoop.shouldRetry(int) was removed; ...)~~>*/` 或 XML 中的 `<!--~~(ZooKeeper 3.4.x is unsupported by Curator 5; ...)~~>-->`；它表示配方已准确找到风险点，但因业务语义不足没有擅自生成代码。

| 不兼容点 | 配方行为 | 验证方式 |
| --- | --- | --- |
| `curator-framework` 版本 | **AUTO**：Maven/Gradle 中受支持的显式源版本升级到 `5.7.1` | 四个表格源版本、直接/属性/managed/profile 与 Gradle 多种声明 before→after |
...
| 显式 ZooKeeper `3.4.x` | **MARK**：精确标记 Maven/Gradle 依赖版本，以及解析后 properties/YAML/XML 中的 `zookeeper.version`/`zookeeper-version` | 构建与配置节点标记、注释/相似 key 负例 |
| 显式但未对齐的其他 Curator 组件 | **MARK**：标记受支持 Maven/Gradle 依赖节点，要求与 framework 统一发布线 | framework/client/recipes/discovery 坐标归属与目标版本负例 |
| 连接状态、ACL、重试幂等、cache 事件、集群滚动升级 | **人工处理**：不存在跨业务通用的一对一代码变换 | 按本文验证清单执行故障与集成测试 |

底层版本配方由本模块的 LST visitor 实现，只接受表格明确给出的源版本 `2.7.1`、`5.2.0`、`5.3.0`、`5.4.0`，不通过 Maven metadata 猜测“更旧”或“兼容”版本。支持：

- Maven `project` 或 profile 的直接 dependencies、`dependencyManagement`，以及可证明安全的本地属性；plugin dependency 和伪造相似 XML 不在归属边界内；
- 仅在真实 `dependencies {}` 块内的标准 Gradle configuration，支持 Groovy 字符串/Map 和 Kotlin DSL 直接字面量；
- 保留 scope/configuration、optional 和 exclusion 等已有元数据；带 classifier/type/ext 的非标准 artifact 会保持不变，推荐配方在实际依赖节点精确 MARK，需按该 artifact 是否真实发布单独决策；
- 表格外旧版、范围、动态值、变量、已经是 `5.7.1` 或更高版本时保持不变，不猜测也不降级；
- 无显式版本，或由外部 parent/BOM/platform/property/catalog 管理的依赖不会被强行写入 `5.7.1`；推荐配方在实际 Curator 依赖节点 MARK，要求修改真正的版本所有者；
- 底层版本 AUTO 不单独修改 `curator-client`、`curator-recipes`、`curator-test`、`curator-x-discovery` 或相似坐标；但安全的共享 Curator 属性会整套对齐，推荐配方会 MARK 显式的未对齐组件；
- `target`、`build`、`out`、`dist`、`generated`、`install`、`.gradle`、`.mvn`、`.idea` 和 `node_modules` 下的生成/安装产物全部 NOOP。

配方不全局扫描替换 Java 字符串、Dockerfile 或普通文本。对配置文件，只解析并 MARK 精确的 ZooKeeper 3.4.x key/value，不自动选择 ZooKeeper 新版本：目标值取决于服务端 ensemble、平台 BOM 和安全基线，不存在普适的确定性替换。

Kotlin DSL 的精确直接坐标即使没有 `GradleProject` marker 也可安全升级，因为 visitor 同时校验 dependency configuration、group、artifact 和完整版本。插值变量、version catalog alias、platform 和任意普通字符串仍保持不变，应在真正拥有版本的文件处理。

Maven active profile 内部的独占属性会原位更新，不会再向顶层凭空增加同名属性。若同名属性存在多个定义，配方无法证明 profile 解析归属，会保持不变。

如果同一个 Maven 属性只控制标准 JAR 形态的 `org.apache.curator:*` 组件，更新属性会让整套 Curator 保持 `5.7.1` 对齐。只要该 token 还出现在非 Curator 依赖、插件配置、项目名称、XML attribute 或其他元数据中，配方就完整保留属性和 framework 声明，避免共享属性误伤；测试同时覆盖真实 Curator 套件共享和无关 metadata 两条路径。
...
Apache Curator 官方 [5.0 Breaking Changes](https://curator.apache.org/docs/breaking-changes) 列出的断点全部落在本次 `2.7.1`→`5.7.1` 路径中；对 `5.2.0`/`5.3.0`/`5.4.0` 源工程，标记仍可用于发现长期保留的旧 API 和未对齐的依赖：
...
- [Apache Curator 2.7.0 `206a590`](https://github.com/apache/curator/blob/206a59043cb94fe51dbd878b080f68d1c0b7595e/curator-framework/src/main/java/org/apache/curator/framework/state/ConnectionStateManager.java#L68)：真实 `new ListenerContainer<>()` 构造形态，用于验证 `StandardListenerManager.standard()` 的 Java AST before→after；
- 同一 Curator 固定提交中的 [`RetryLoop.shouldRetry`](https://github.com/apache/curator/blob/206a59043cb94fe51dbd878b080f68d1c0b7595e/curator-framework/src/main/java/org/apache/curator/framework/imps/CuratorFrameworkImpl.java#L512) 和 [delete 路径](https://github.com/apache/curator/blob/206a59043cb94fe51dbd878b080f68d1c0b7595e/curator-framework/src/main/java/org/apache/curator/framework/imps/DeleteBuilderImpl.java#L254)：用于验证已删除 retry 分类 API 的精确 MARK；
- 同一 Curator 固定提交中的 [`NodeCache`](https://github.com/apache/curator/blob/206a59043cb94fe51dbd878b080f68d1c0b7595e/curator-recipes/src/main/java/org/apache/curator/framework/recipes/cache/NodeCache.java#L63)：用于验证旧 cache 类型 MARK 与用户同名类 NOOP。

测试风格参考 OpenRewrite 官方 Apache 2.0 仓库 [`rewrite-java-dependencies` 在 `decb8db` 的 `UpgradeDependencyVersionTest`](https://github.com/openrewrite/rewrite-java-dependencies/blob/decb8db/src/test/java/org/openrewrite/java/dependencies/UpgradeDependencyVersionTest.java)。当前 53 个测试覆盖四个表格源版本、Maven 直接/独占属性/Curator 套件共享属性/managed/profile、Gradle Groovy 字符串/Map/Kotlin DSL，以及 plugin/fake XML、DSL 块外、变量/动态版本、重复/外部属性、versionless parent/BOM/platform、classifier/type/ext 变体、生成/安装目录和相似坐标的 NOOP。Java 测试包含 `ListenerContainer` before→after、所有已知删除 API 家族、旧 cache 与同名应用 API 负例；构建/配置测试包含 Curator 组件未对齐、ZooKeeper 3.4.x、properties/YAML/XML 精确 key 与注释/相似 key 负例。推荐配方还有两轮 AUTO/MARK 幂等性和 recipe discovery/validation 测试。
