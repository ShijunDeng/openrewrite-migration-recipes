...
/** Marks exact non-build configuration values that pin an unsupported ZooKeeper 3.4 baseline. */
public final class FindCurator5ConfigurationRisks extends Recipe {
    ...
    private static final String MESSAGE =
            "Curator 5 cannot run with ZooKeeper 3.4.x; migrate the owning client/server baseline and verify ensemble rolling compatibility, authentication, ACLs, watches, sessions, and container nodes";

    @Override
    public String getDisplayName() {
        return "Find Curator 5 configuration risks";
    }

    @Override
    public String getDescription() {
        return "Mark exact zookeeper.version/zookeeper-version values pinned to 3.4.x in parsed properties, YAML, " +
               "and non-POM XML without changing platform configuration.";
    }
                ...
                if (tree instanceof Properties.File properties) {
                    return new PropertiesIsoVisitor<ExecutionContext>() {
                        ...
                        public Properties.Entry visitEntry(Properties.Entry entry, ExecutionContext executionContext) {
                            ...
                            if (!KEYS.contains(visited.getKey()) ||
                                !ZOOKEEPER_34.matcher(visited.getValue().getText().trim()).matches()) return visited;
                            return mark(visited, MESSAGE);
                        }
