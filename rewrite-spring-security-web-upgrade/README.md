...
无法替业务做决定的位置写入可检索的 `SearchResult` 标记。源码、配置和 Java
基线迁移只会进入“最近构建根明确拥有一个白名单源版本”的工程；无关工程完全
`NOOP`。
...
| 任何其他高于 `6.5.11` 的固定版本 | 保留原文，写入相同禁止降级标记 |
...
高版本比较优先复用 OpenRewrite Core `LatestRelease`，超长数字段再使用
`BigInteger` 回退；`6.5.11.1`、`6.5.11-sp1` 与
`6.5.11.999999999999999999999` 都会得到精确禁止降级标记。配方不会把 7.x
降级到 6.5.11，也不会把“升级到当前最新版本”解释为授权扩大白名单。
...
| `UpgradeSelectedSpringSecurityWebBuildToJava17` | AUTO | 仅对白名单工程运行官方 `UpgradeJavaVersion(17)` |
| `MigrateSelectedDeterministicSpringSecurityWeb6` | AUTO | 按扫描到的精确源版本分派官方 5.7～6.2 与 Jakarta 迁移 |
| `MigrateDeterministicSpringSecurityWeb6` | AUTO（专家入口） | 不带项目门控的官方叶子组合；推荐入口不会直接调用它 |
...
| `MigrateSpringSecurityWebTo6_5_11` | AUTO + MARK | 扫描构建所有权后执行受门控的推荐迁移 |
...
推荐入口先扫描所有 `pom.xml`、`build.gradle(.kts)` 和
`settings.gradle(.kts)` 构建根，并给白名单工程的文件附加不打印的 marker。
同一构建根出现不同源版本、白名单与非白名单混装、变体或不明确 owner 时，不会
获得 marker。嵌套 Maven/Gradle 根是硬边界；不含本依赖的子工程不会继承外层
AUTO 资格。由父 POM/BOM 或 version catalog 管理的子工程只做 owner MARK，
不会猜测继承关系。

源版本分派规则：

- 5.x 白名单工程执行官方 5.7、5.8、6.0、6.1、6.2 兼容叶子及
  `javax.servlet` → `jakarta.servlet`；
- 6.2.x 白名单工程只补运行官方 6.2 `apply(..)` → `with(..)` 叶子；
- 6.3.x～6.5.x 不重放 5.x/Jakarta 历史迁移，只升级依赖并执行风险定位。

因此，一个 6.2+ 工程中与 Spring Security 无关的遗留 `javax.servlet` 用法不会
被推荐配方机械改写。
...
| Jakarta Servlet API | `6.0.0` | 5.x 白名单工程 AUTO 坐标/包迁移；其余工程 MARK 混装 |
| Micrometer | `1.15.12` | MARK observation/core 错位 |
| context-propagation | `1.1.4` | MARK context 传播错位 |

对白名单工程，推荐入口复用固定官方 `UpgradeJavaVersion(17)`，自动更新其能够
识别的 Maven/Gradle Java 基线以及关联的 Jenkins、容器镜像、SDKMAN/Kotlin
target 配置；仍无法证明归属或无法自动处理的低版本配置才会被标记。
...
| 6.3、6.4、6.5 aggregate | 不直接激活 | 固定版本中只有宽泛 Security/Authorization Server 版本升级，没有遗漏可复用的源码叶子 |
...
推荐入口另用真实 before/after 断言 5.x 会运行兼容叶子、6.2+ 不会重放
5.x/Jakarta 迁移、无关与嵌套非目标工程完全 NOOP。
...
Java 17 方面，官方 `UpgradeToJava17` aggregate 还包含与本任务无关的通用 API、
插件和第三方依赖升级，因此不整体激活；推荐入口直接复用其固定底层
`org.openrewrite.java.migrate.UpgradeJavaVersion`，参数严格设为 `17`，并受
精确项目 marker 门控。
...
- Maven、Groovy Gradle、Kotlin Gradle 的 owner、profile、dependency management、
  shared property、variant、nested scope、嵌套构建根与生成目录；
- 推荐入口的精确项目门控、release-aware option 运行时绑定、无关工程 NOOP、
  5.x/6.2+ 分派、官方 Java 17 构建升级及 Java 21 禁止降级；
