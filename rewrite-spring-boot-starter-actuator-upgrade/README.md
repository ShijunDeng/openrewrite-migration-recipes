...
本模块在实现和回查时固定审计
[`rewrite-spring` 6.35.0 / `d28afcb6`](https://github.com/openrewrite/rewrite-spring/tree/d28afcb6661ad413539056de0936c5489ff9d8ee)
以及工程使用的 OpenRewrite core `8.87.5`。实际解析的
`rewrite-spring-6.35.0.jar` SHA-256 为
`27df444210c8bfee7e9d0f04d6d6f7986d2bee36bcd472d8307912613e93e98b`。
`rewrite-spring` 是运行时依赖，采用 Moderne Source Available License；
本仓库没有复制其实现源码。
...
| `ChangePackage` / `ChangeType` | 直接复用 | Jakarta 源码迁移只编排官方 core 原语，不维护自定义 Java AST 改写器 |
...
路径保护，以及依赖业务上下文的精确风险 marker。组合测试不仅断言最终
文本，还断言运行时 recipe tree 中确实包含官方
`ChangeSpringPropertyKey` 和
`SpringBootProperties_3_4_EnabledToAccess`，并单独覆盖官方 recipe 对
`@Value`、properties、嵌套 YAML、非 literal access 和两周期幂等的行为。
...
本模块当前执行 146 项测试，覆盖 14 个精确源版本、Maven/Gradle owner、
