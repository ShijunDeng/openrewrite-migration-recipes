...
| `com.huawei.clouds.openrewrite.springweb.MigrateDeterministicSpringWeb6Java` | 声明式组合经过审计的 Core / `rewrite-spring` 精确叶子和必要的窄范围补丁 |
...
| `MethodArgumentNotValidException.errorsToStringList(...)` / `resolveErrorMessages(...)` → `BindErrorUtils.resolve(...)` | 精确 method owner 和 overload；保留输入 error 顺序、message source 与 locale | 6.1.13 [旧方法体](https://github.com/spring-projects/spring-framework/blob/f083962fd8d96a46f89d11e375e50a14ea0243fc/spring-web/src/main/java/org/springframework/web/bind/MethodArgumentNotValidException.java#L94-L132)与官方 [`MigrateMethodArgumentNotValidExceptionErrorMethod`](https://github.com/openrewrite/rewrite-spring/blob/d28afcb6661ad413539056de0936c5489ff9d8ee/src/main/java/org/openrewrite/java/spring/framework/MigrateMethodArgumentNotValidExceptionErrorMethod.java)输出逐表达式一致 |
| HTTP client observation `HighCardinalityKeyNames.CLIENT_NAME` → `LowCardinalityKeyNames.CLIENT_NAME` | 精确 nested enum 字段归属；`HTTP_URL` 和 reactive client 常量不匹配 | 6.1.13 [旧常量](https://github.com/spring-projects/spring-framework/blob/f083962fd8d96a46f89d11e375e50a14ea0243fc/spring-web/src/main/java/org/springframework/http/client/observation/ClientHttpObservationDocumentation.java#L128-L154)与固定官方 6.2 声明 |
...
| `UriComponentsBuilder.fromHttpRequest(request)` → `ForwardedHeaderUtils.adaptFromForwardedHeaders(request.getURI(), request.getHeaders())` | `request` 必须是稳定 identifier，避免复制带副作用表达式 | 旧 `fromHttpRequest` 与目标 `ForwardedHeaderUtils` 实现固定在上述 Spring commits |
...
`fromHttpRequest(nextRequest())` / `parseForwardedFor(nextRequest(), …​)` 保持不动并由
MARK 引导人工处理。
...
审计基线固定到实际 Maven Central 字节，不使用“当前最新版”：

| 组件 | 固定源码/manifest | Maven Central JAR SHA-256 | 审计用途 |
| --- | --- | --- | --- |
| `rewrite-spring:6.35.0` | [`d28afcb6`](https://github.com/openrewrite/rewrite-spring/tree/d28afcb6661ad413539056de0936c5489ff9d8ee) | `27df444210c8bfee7e9d0f04d6d6f7986d2bee36bcd472d8307912613e93e98b` | Framework 5.2/6.0/6.1/6.2 声明、五个采用的 Java 叶子及其官方测试 |
| `rewrite-core:8.87.5` | [`b3008cc4`](https://github.com/openrewrite/rewrite/tree/b3008cc4a1f0c43f562da16e5933a2a56d9bc568) | `a7ff59eebc8072353ec5c3aee3e2033bc69a844b3c9ce2e9be8d4adaec10cbf8` | recipe runtime 与声明式组合 |
| `rewrite-java:8.87.5` | Central JAR manifest 实际记录 `8.88.0-SNAPSHOT` / [`91e23c28`](https://github.com/openrewrite/rewrite/tree/91e23c2858176877428ddc03e146d2bb023217a8) | `a378253fe0c0865ab39d1743e468fe3d2557d7760e0a6897de294ca18ea90043` | `ChangePackage`、`ChangeType`、`ChangeMethodName`、`ReplaceConstantWithAnotherConstant` 的实际运行代码与测试；README 不把该字节错误归到 `b3008cc4` |

`MigrateDeterministicSpringWeb6Java` 直接声明 Core 节点和官方叶子；原
`MigrateSpringWebDeterministicJava` / `ApplyOfficialSpringWebMigrations` 两层本地 delegate
已删除，只保留官方无法安全覆盖的窄范围调用补丁。统一 precondition 对整个组合执行
generated/cache 路径排除。
...
| `MigrateMethodArgumentNotValidExceptionErrorMethod` | **直接复用** | 三个目标方法在 6.1.13 的方法体与官方输出一致，目标 6.2.19 已移除旧入口 |
| HTTP client observation `CLIENT_NAME` 的官方 Core 节点 | **按固定参数直接复用** | 只替换 `spring-web` blocking client 常量；reactive client 常量属于 WebFlux 模块 |
| Core `ChangePackage` / `ChangeType` / `ChangeMethodName` | **声明式参数化复用** | 五个 Web 相关 Jakarta 包、Reactor resource 类型和 response wrapper alias 都有精确参数；不激活 Jakarta aggregate |
| `UpgradeSpringFramework_5_2` / `_5_3` / `_6_0` / `_6_1` / `_6_2` 与通用依赖 selector | **不复用 aggregate** | 固定 [`spring-framework-62.yml`](https://github.com/openrewrite/rewrite-spring/blob/d28afcb6661ad413539056de0936c5489ff9d8ee/src/main/resources/META-INF/rewrite/spring-framework-62.yml)会递归升级 `org.springframework:*` 到通配 `6.2.x`，越过单 artifact、Excel 精确白名单、精确 `6.2.19` 和禁止降级边界 |
| `JakartaEE10` / `UpgradeJavaVersion` | **不复用 aggregate** | 它们会改写未获授权的 EE 依赖、插件、XML、容器或全工程 Java 基线；本模块只 AUTO 五个类型归属的 Web 相关包，构建与容器继续 MARK |
...
| `HttpComponents*Timeout`、移除 `setOutputStreaming`、Jetty client 类型替换 | **保留 MARK / 排除** | timeout owner、buffering 和 client 引擎会改变连接池、内存、TLS、event-loop 或生命周期，不能作为等价叶子执行 |
| `MigrateBase64Utils`、`MigrateSpringAssert`、MVC / WebFlux 专属配方 | **排除** | Spring Core、MVC `ResponseEntityExceptionHandler`、WebFlux `HandlerResult` / writer / exchange 不属于 `spring-web` 叶子职责 |
| Spring Boot 与 trailing-slash 系列配方 | **排除** | Boot 基线、路由、安全和 SEO 策略属于各自模块及业务决策 |
...
`SpringWebOfficialRecipeReuseTest` 会展开运行时 recipe tree，锁定 14 个直接节点及全部参数，
并断言 Framework/Boot/Jakarta/WebFlux aggregate、通用依赖 selector、危险 URI 整体配方
和跨模块叶子不存在；这不是仅对 YAML 文本做字符串检查。
...
| URI / forwarded / CORS | 删除的 URI handler、`fromHttpUrl` / `fromUriString`、带副作用的 `fromHttpRequest` / `parseForwardedFor` 和精确 CORS setter；验证 RFC/WHATWG parser、信任边界、encoding 和 preflight |
...
- [`apache/kylin@b5b94b51` 的 `BaseFilter`](https://github.com/apache/kylin/blob/b5b94b51abaf83d68088e4fcab606d2a6530e54d/src/server/src/main/java/org/apache/kylin/rest/BaseFilter.java)：真实 `HttpStatusCodeException.getRawStatusCode()` 子类调用，验证官方 owner 继承匹配和 `getStatusCode().value()` 输出；仓库固定 [Apache-2.0 LICENSE](https://github.com/apache/kylin/blob/b5b94b51abaf83d68088e4fcab606d2a6530e54d/LICENSE)；
- [`hapifhir/hapi-fhir@28652be3` 的 `ApacheProxyAddressStrategy`](https://github.com/hapifhir/hapi-fhir/blob/28652be3664a86c22d5ceba96a535a8c086ee617/hapi-fhir-server/src/main/java/ca/uhn/fhir/rest/server/ApacheProxyAddressStrategy.java)：真实稳定 identifier `UriComponentsBuilder.fromHttpRequest(requestWrapper)`，验证只迁移单次求值安全形态；源文件头固定 Apache-2.0；
...
真实仓库只能证明“用法存在”，官方固定源码才证明兼容性。测试结构参考实际
`rewrite-java:8.87.5` artifact manifest commit `91e23c28` 的
[`ChangePackageTest`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java-test/src/test/java/org/openrewrite/java/ChangePackageTest.java)、
[`ChangeTypeTest`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java-test/src/test/java/org/openrewrite/java/ChangeTypeTest.java)、
[`ChangeMethodNameTest`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java-test/src/test/java/org/openrewrite/java/ChangeMethodNameTest.java)、
[`ReplaceConstantWithAnotherConstantTest`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java-test/src/test/java/org/openrewrite/java/ReplaceConstantWithAnotherConstantTest.java)
和固定 `rewrite-spring` 官方用例。
...
- 五个官方 `rewrite-spring` class 与精确 Core 参数的运行时树，以及 media type、
  ResponseStatus、MethodArgumentNotValid、observation、RestClientResponseException 子类、
  ClientHttpResponse 和隐式接收者上游缺口的实际变换；
- Spring Web/Context/Core `6.2.19` 与 Micrometer `1.15.12` 真实目标 artifact classpath
  上的完整类型归属、幂等正例和相邻 API 负例；
...
- 五个真实仓库裁剪测试 fixture、一个人工交叉检查 fixture、五个公共配方发现和
  本地/官方 aggregate 执行顺序。
