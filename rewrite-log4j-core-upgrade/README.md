...
- OpenRewrite Core `8.87.5` 固定到 [`b3008cc4`](https://github.com/openrewrite/rewrite/tree/b3008cc4a1f0c43f562da16e5933a2a56d9bc568)。
- `rewrite-logging-frameworks:3.30.0` 的制品 manifest 固定到 [`c357a720`](https://github.com/openrewrite/rewrite-logging-frameworks/tree/c357a7209d721078dc942a777b1d8cc95941f722)。该制品只以 `test` scope 加载，用于验证真实运行时发现和组合结构，不进入本模块生产依赖。

直接复用的官方原语是 Core 的
[`ChangeMethodName`](https://github.com/openrewrite/rewrite/blob/b3008cc4a1f0c43f562da16e5933a2a56d9bc568/rewrite-java/src/main/java/org/openrewrite/java/ChangeMethodName.java)：
...
本地包装只排除生成/安装/缓存目录，不再复制官方的方法名、方法类型更新逻辑。`MigrateRemovedNoLookupsOption` 仍保留为窄范围本地能力：固定
...
| 官方能力 | 拒绝原因 |
|---|---|
| `UpgradeLog4J2DependencyVersion` | `artifactId: '*'`、浮动 `2.x` 且 `overrideManagedVersion: true`，会扩大到全部 Log4j 家族并改写托管所有者，不满足 10 个源版本、单一 artifact、精确 `2.25.5` 和禁止降级边界 |
| `Log4j1ToLog4j2`、`CommonsLoggingToLog4j`、`JulToLog4j`、`Slf4jToLog4j` | 输入框架不同，本任务源端已经是 Log4j Core 2.x |
| `LoggerSetLevelToConfiguratorRecipe` | 处理 `org.apache.log4j.Logger`（Log4j 1.x），不是 Core 2.x patch；测试只验证该官方 class recipe 能从运行时 classpath 激活 |
...
`mvn -f rewrite-log4j-core-upgrade/pom.xml clean verify` 执行 **224** 项测试（0 failure / 0 error / 0 skip）。覆盖：
...
- 三个 Core `ChangeMethodName` 委托的精确参数、官方 logging class/composition 运行时激活、宽泛官方配方排除和官方原语与本地缺口的 before/after 组合。
