...
/** Upgrades only the exact JSON-java source versions selected by the workbook. */
public final class UpgradeSelectedOrgJsonDependency extends Recipe {
    private static final String PREFIX = OrgJsonSupport.GROUP + ":" + OrgJsonSupport.ARTIFACT + ":";
    private static final Pattern PROPERTY_REFERENCE = Pattern.compile("\\$\\{([^}]+)}");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected org.json dependencies to 20250107";
    }

    @Override
    public String getDescription() {
        return "Upgrades only exact org.json:json versions 20160810, 20180813, 20190722, 20200518, 20220320, 20230227, and 3.11.2 in owned standard Maven or Gradle declarations.";
    }
    ...
    private static G.CompilationUnit migrateGroovy(G.CompilationUnit source, ExecutionContext ctx) {
        return (G.CompilationUnit) new GroovyIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext executionContext) {
                boolean dependency = OrgJsonSupport.isGradleDependencyInvocation(getCursor(), method);
                J.MethodInvocation visited = super.visitMethodInvocation(method, executionContext);
                if (!dependency) return visited;
                if (coordinate(OrgJsonSupport.mapValue(visited, "group"), OrgJsonSupport.mapValue(visited, "name")) &&
                    OrgJsonSupport.SOURCES.contains(OrgJsonSupport.mapValue(visited, "version")) && !OrgJsonSupport.hasVariant(visited)) {
                    return visited.withArguments(visited.getArguments().stream().map(argument ->
                            argument instanceof G.MapEntry entry && "version".equals(OrgJsonSupport.mapKey(entry)) &&
                            entry.getValue() instanceof J.Literal literal ?
                                    entry.withValue(OrgJsonSupport.replaceLiteral(literal, OrgJsonSupport.TARGET)) : argument).toList());
                }
                return visited.withArguments(visited.getArguments().stream().map(argument ->
            ...
            public J.Literal visitLiteral(J.Literal literal, ExecutionContext executionContext) {
                boolean dependency = OrgJsonSupport.isDirectDependencyLiteral(getCursor());
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Set<PropertyOwner> safe = targetReferences.keySet().stream()
                .filter(owner -> values.get(owner) != null && OrgJsonSupport.SOURCES.contains(values.get(owner)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                Xml.Tag visited = super.visitTag(tag, executionContext);
                if (OrgJsonSupport.isMavenPropertyDefinition(getCursor(), visited) && safe.contains(propertyOwner(getCursor(), visited.getName())) &&
                    visited.getValue().map(String::trim).filter(OrgJsonSupport.SOURCES::contains).isPresent()) {
                    return visited.withValue(OrgJsonSupport.TARGET);
                }
                if (OrgJsonSupport.isOrgJsonDependency(getCursor(), visited) && OrgJsonSupport.standardJar(visited) &&
                    visited.getChildValue("version").map(String::trim).filter(OrgJsonSupport.SOURCES::contains).isPresent()) {
                    return visited.withChildValue("version", OrgJsonSupport.TARGET);
                }
                return visited;
            }
        }.visitNonNull(document, ctx);
    ...
    private static boolean coordinate(String group, String artifact) {
        return OrgJsonSupport.GROUP.equals(group) && OrgJsonSupport.ARTIFACT.equals(artifact);
    }

    private static G.MapLiteral upgradeMap(G.MapLiteral map) {
        if (!coordinate(OrgJsonSupport.mapValue(map, "group"), OrgJsonSupport.mapValue(map, "name")) ||
            !OrgJsonSupport.SOURCES.contains(OrgJsonSupport.mapValue(map, "version")) || OrgJsonSupport.hasVariant(map)) return map;
        return map.withElements(map.getElements().stream().map(entry ->
                "version".equals(OrgJsonSupport.mapKey(entry)) && entry.getValue() instanceof J.Literal literal ?
                        entry.withValue(OrgJsonSupport.replaceLiteral(literal, OrgJsonSupport.TARGET)) : entry).toList());
    }
    ...
    private static J.Literal upgradeCoordinate(J.Literal literal) {
        ...
        return OrgJsonSupport.SOURCES.contains(version) ? OrgJsonSupport.replaceLiteral(literal, PREFIX + OrgJsonSupport.TARGET) : literal;
    }
