...
final class CommonsPool2Support {
    static final String GROUP = "org.apache.commons";
    static final String ARTIFACT = "commons-pool2";
    static final String TARGET = "2.13.1";
    static final Set<String> SOURCES = Set.of("2.6.0", "2.8.0", "2.8.1", "2.9.0", "2.10.0", "2.11.1");
    ...
    static boolean isCommonsPool2Dependency(Cursor cursor, Xml.Tag tag) {
    ...
    static boolean isMavenPropertyDefinition(Cursor cursor, Xml.Tag tag) {
        ...
        if (!(properties.getValue() instanceof Xml.Tag p) || !"properties".equals(p.getName()) || "properties".equals(tag.getName())) return false;
    ...
    private static boolean profile(Cursor cursor) {
        ...
        return profiles.getValue() instanceof Xml.Tag p && "profiles".equals(p.getName()) && project(profiles.getParentTreeCursor());
    }
    ...
    static String mapValue(J.MethodInvocation invocation, String key) {
        return invocation.getArguments().stream().filter(G.MapEntry.class::isInstance).map(G.MapEntry.class::cast)
                ...
                .map(J.Literal.class::cast).map(J.Literal::getValue).filter(String.class::isInstance).map(String.class::cast)
