...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/python/pypi-onnxruntime` |
| Maven artifactId | `migration-spec-python-pypi-onnxruntime` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `onnxruntime` |
| Catalog canonical identity | `onnxruntime`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `python` |
| Excel 原始语言 | `python` |
| 目标版本 | `1.17.1` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4320 | 4319 | `onnxruntime` | python | `1.13.1` | `1.17.1` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 4321 | 4320 | `onnxruntime` | python | `1.15.1` | `1.17.1` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4320 1.13.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #4321 1.15.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.17.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
