...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/python/pypi-configparser` |
| Maven artifactId | `migration-spec-python-pypi-configparser` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `configparser` |
| Catalog canonical identity | `configparser`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `python` |
| Excel 原始语言 | `python` |
| 目标版本 | `7.2.0` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1899 | 1898 | `configparser` | python | `6.0.0` | `7.2.0` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1899 6.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.2.0` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
