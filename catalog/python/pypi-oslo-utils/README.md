...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/python/pypi-oslo-utils` |
| Maven artifactId | `migration-spec-python-pypi-oslo-utils` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `oslo-utils` |
| Catalog canonical identity | `oslo-utils`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `python` |
| Excel 原始语言 | `python` |
| 目标版本 | `9.1.0` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 825 | 824 | `oslo-utils` | python | `3.42.1` | `9.1.0` | 1 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.42.1`。
- 升级候选边：`3.42.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #825 3.42.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `9.1.0` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
