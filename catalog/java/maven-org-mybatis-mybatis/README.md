...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-mybatis-mybatis` |
| Maven artifactId | `migration-spec-java-maven-org-mybatis-mybatis` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.mybatis:mybatis` |
| Catalog canonical identity | `org.mybatis:mybatis`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.5.19` |
| Excel 迁移边 | 8 |
| 涉及微服务数 | 最大可见值 `95`；不同版本行不累加 |
| 分桶 | `B1_Patch直升` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2370 | 2369 | `org.mybatis:mybatis` | java | `3.5.10` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2371 | 2370 | `org.mybatis:mybatis` | java | `3.5.11` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2372 | 2371 | `org.mybatis:mybatis` | java | `3.5.12` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2373 | 2372 | `org.mybatis:mybatis` | java | `3.5.13` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2374 | 2373 | `org.mybatis:mybatis` | java | `3.5.2` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2375 | 2374 | `org.mybatis:mybatis` | java | `3.5.6` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2376 | 2375 | `org.mybatis:mybatis` | java | `3.5.7` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2377 | 2376 | `org.mybatis:mybatis` | java | `3.5.9` | `3.5.19` | 95 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.5.10`, `3.5.11`, `3.5.12`, `3.5.13`, `3.5.2`, `3.5.6`, `3.5.7`, `3.5.9`。
- 升级候选边：`3.5.10`, `3.5.11`, `3.5.12`, `3.5.13`, `3.5.2`, `3.5.6`, `3.5.7`, `3.5.9`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2370 3.5.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2371 3.5.11 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2372 3.5.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2373 3.5.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2374 3.5.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2375 3.5.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2376 3.5.7 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2377 3.5.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.5.19` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
