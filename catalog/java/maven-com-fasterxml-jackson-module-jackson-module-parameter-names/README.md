# com.fasterxml.jackson.module:jackson-module-parameter-names 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-fasterxml-jackson-module-jackson-module-parameter-names` |
| Maven artifactId | `migration-spec-java-maven-com-fasterxml-jackson-module-jackson-module-parameter-names` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.fasterxml.jackson.module:jackson-module-parameter-names` |
| Catalog canonical identity | `com.fasterxml.jackson.module:jackson-module-parameter-names`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.21.2` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2813 | 2812 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.10.3` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2814 | 2813 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.13.2` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2815 | 2814 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.14.0-rc1` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2816 | 2815 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.14.2` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2817 | 2816 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.15.1` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2818 | 2817 | `com.fasterxml.jackson.module:jackson-module-parameter-names` | java | `2.15.2` | `2.21.2` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2813 2.10.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2814 2.13.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2815 2.14.0-rc1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2816 2.14.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2817 2.15.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2818 2.15.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.21.2` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
