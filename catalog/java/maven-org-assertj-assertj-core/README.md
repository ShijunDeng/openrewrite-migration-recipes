...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-assertj-assertj-core` |
| Maven artifactId | `migration-spec-java-maven-org-assertj-assertj-core` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.assertj:assertj-core` |
| Catalog canonical identity | `org.assertj:assertj-core`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.27.7` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2778 | 2777 | `org.assertj:assertj-core` | java | `3.19.0` | `3.27.7` | 27 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2779 | 2778 | `org.assertj:assertj-core` | java | `3.21.0` | `3.27.7` | 27 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2780 | 2779 | `org.assertj:assertj-core` | java | `3.22.0` | `3.27.7` | 27 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2781 | 2780 | `org.assertj:assertj-core` | java | `3.24.2` | `3.27.7` | 27 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2782 | 2781 | `org.assertj:assertj-core` | java | `3.26.3` | `3.27.7` | 27 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2783 | 2782 | `org.assertj:assertj-core` | java | `3.27.6` | `3.27.7` | 27 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.19.0`, `3.21.0`, `3.22.0`, `3.24.2`, `3.26.3`, `3.27.6`。
- 升级候选边：`3.19.0`, `3.21.0`, `3.22.0`, `3.24.2`, `3.26.3`, `3.27.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2783 3.27.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.27.7` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2778 3.19.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2779 3.21.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2780 3.22.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2781 3.24.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2782 3.26.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.27.7` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
