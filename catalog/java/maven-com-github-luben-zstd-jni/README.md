# com.github.luben:zstd-jni 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-luben-zstd-jni` |
| Maven artifactId | `migration-spec-java-maven-com-github-luben-zstd-jni` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.luben:zstd-jni` |
| Catalog canonical identity | `com.github.luben:zstd-jni`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.5.7-6` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2701 | 2700 | `com.github.luben:zstd-jni` | java | `1.5.2-2` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2702 | 2701 | `com.github.luben:zstd-jni` | java | `1.5.2-3` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2703 | 2702 | `com.github.luben:zstd-jni` | java | `1.5.4-1` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2704 | 2703 | `com.github.luben:zstd-jni` | java | `1.5.5-2` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2705 | 2704 | `com.github.luben:zstd-jni` | java | `1.5.5-4` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2706 | 2705 | `com.github.luben:zstd-jni` | java | `1.5.5-5` | `1.5.7-6` | 29 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.5.2-2`, `1.5.2-3`, `1.5.4-1`, `1.5.5-2`, `1.5.5-4`, `1.5.5-5`。
- 升级候选边：`1.5.2-2`, `1.5.2-3`, `1.5.4-1`, `1.5.5-2`, `1.5.5-4`, `1.5.5-5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2701 1.5.2-2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2702 1.5.2-3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2703 1.5.4-1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2704 1.5.5-2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2705 1.5.5-4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2706 1.5.5-5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.5.7-6` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
