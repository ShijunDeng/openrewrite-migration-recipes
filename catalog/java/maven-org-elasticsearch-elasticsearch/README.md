# org.elasticsearch:elasticsearch 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-elasticsearch-elasticsearch` |
| Maven artifactId | `migration-spec-java-maven-org-elasticsearch-elasticsearch` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.elasticsearch:elasticsearch` |
| Catalog canonical identity | `org.elasticsearch:elasticsearch`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `7.17.12` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 487 | 486 | `org.elasticsearch:elasticsearch` | java | `1.17.6` | `7.17.12` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 3485 | 3484 | `org.elasticsearch:elasticsearch` | java | `7.10.2` | `7.17.12` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3486 | 3485 | `org.elasticsearch:elasticsearch` | java | `7.10.2-hw-ei-315005` | `7.17.12` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3487 | 3486 | `org.elasticsearch:elasticsearch` | java | `7.17.9` | `7.17.12` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3488 | 3487 | `org.elasticsearch:elasticsearch` | java | `7.9.3` | `7.17.12` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.17.6`, `7.10.2`, `7.10.2-hw-ei-315005`, `7.17.9`, `7.9.3`。
- 升级候选边：`1.17.6`, `7.10.2`, `7.10.2-hw-ei-315005`, `7.17.9`, `7.9.3`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3487 7.17.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.17.12` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3485 7.10.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3486 7.10.2-hw-ei-315005 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3488 7.9.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.17.12` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-003 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #487 1.17.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.17.12` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
