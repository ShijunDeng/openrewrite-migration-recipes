# org.eclipse.lsp4j:org.eclipse.lsp4j 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-eclipse-lsp4j-org-eclipse-lsp4j` |
| Maven artifactId | `migration-spec-java-maven-org-eclipse-lsp4j-org-eclipse-lsp4j` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.eclipse.lsp4j:org.eclipse.lsp4j` |
| Catalog canonical identity | `org.eclipse.lsp4j:org.eclipse.lsp4j`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `0.24.0` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4084 | 4083 | `org.eclipse.lsp4j:org.eclipse.lsp4j` | java | `0.12.0` | `0.24.0` | 2 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 4085 | 4084 | `org.eclipse.lsp4j:org.eclipse.lsp4j` | java | `0.20.0` | `0.24.0` | 2 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`0.12.0`, `0.20.0`。
- 升级候选边：`0.12.0`, `0.20.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4084 0.12.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #4085 0.20.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `0.24.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
