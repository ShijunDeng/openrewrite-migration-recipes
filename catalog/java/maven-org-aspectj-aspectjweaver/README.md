# org.aspectj:aspectjweaver 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-aspectj-aspectjweaver` |
| Maven artifactId | `migration-spec-java-maven-org-aspectj-aspectjweaver` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.aspectj:aspectjweaver` |
| Catalog canonical identity | `org.aspectj:aspectjweaver`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.9.25.1` |
| Excel 迁移边 | 8 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2534 | 2533 | `org.aspectj:aspectjweaver` | java | `1.9.19` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2535 | 2534 | `org.aspectj:aspectjweaver` | java | `1.9.4` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2536 | 2535 | `org.aspectj:aspectjweaver` | java | `1.9.5` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2537 | 2536 | `org.aspectj:aspectjweaver` | java | `1.9.6` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2538 | 2537 | `org.aspectj:aspectjweaver` | java | `1.9.7` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2539 | 2538 | `org.aspectj:aspectjweaver` | java | `1.9.8` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2540 | 2539 | `org.aspectj:aspectjweaver` | java | `1.9.9` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2541 | 2540 | `org.aspectj:aspectjweaver` | java | `1.9.9.1` | `1.9.25.1` | 48 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.9.19`, `1.9.4`, `1.9.5`, `1.9.6`, `1.9.7`, `1.9.8`, `1.9.9`, `1.9.9.1`。
- 升级候选边：`1.9.19`, `1.9.4`, `1.9.5`, `1.9.6`, `1.9.7`, `1.9.8`, `1.9.9`, `1.9.9.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2534 1.9.19 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2535 1.9.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2536 1.9.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2537 1.9.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2538 1.9.7 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2539 1.9.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2540 1.9.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2541 1.9.9.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.9.25.1` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
