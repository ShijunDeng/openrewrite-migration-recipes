# org.jacoco:org.jacoco.agent 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-jacoco-org-jacoco-agent` |
| Maven artifactId | `migration-spec-java-maven-org-jacoco-org-jacoco-agent` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.jacoco:org.jacoco.agent` |
| Catalog canonical identity | `org.jacoco:org.jacoco.agent`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `0.8.11` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3258 | 3257 | `org.jacoco:org.jacoco.agent` | java | `0.8.10` | `0.8.11` | 12 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3259 | 3258 | `org.jacoco:org.jacoco.agent` | java | `0.8.7` | `0.8.11` | 12 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3260 | 3259 | `org.jacoco:org.jacoco.agent` | java | `0.8.8` | `0.8.11` | 12 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`0.8.10`, `0.8.7`, `0.8.8`。
- 升级候选边：`0.8.10`, `0.8.7`, `0.8.8`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3258 0.8.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3259 0.8.7 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3260 0.8.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `0.8.11` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
