# org.springframework.boot:spring-boot-starter-data-mongodb 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-springframework-boot-spring-boot-starter-data-mongodb` |
| Maven artifactId | `migration-spec-java-maven-org-springframework-boot-spring-boot-starter-data-mongodb` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.springframework.boot:spring-boot-starter-data-mongodb` |
| Catalog canonical identity | `org.springframework.boot:spring-boot-starter-data-mongodb`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.5.15` |
| Excel 迁移边 | 7 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1460 | 1459 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.2.6.RELEASE` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1461 | 1460 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.5.1` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1462 | 1461 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.7.0` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1463 | 1462 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.7.10` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1464 | 1463 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.7.11` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1465 | 1464 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.7.12` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1466 | 1465 | `org.springframework.boot:spring-boot-starter-data-mongodb` | java | `2.7.5` | `3.5.15` | 13 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.2.6.RELEASE`, `2.5.1`, `2.7.0`, `2.7.10`, `2.7.11`, `2.7.12`, `2.7.5`。
- 升级候选边：`2.2.6.RELEASE`, `2.5.1`, `2.7.0`, `2.7.10`, `2.7.11`, `2.7.12`, `2.7.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1460 2.2.6.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1461 2.5.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1462 2.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1463 2.7.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1464 2.7.11 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1465 2.7.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1466 2.7.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.5.15` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
