# org.springframework:spring-messaging 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-springframework-spring-messaging` |
| Maven artifactId | `migration-spec-java-maven-org-springframework-spring-messaging` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.springframework:spring-messaging` |
| Catalog canonical identity | `org.springframework:spring-messaging`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `6.2.19` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1947 | 1946 | `org.springframework:spring-messaging` | java | `5.2.5.RELEASE` | `6.2.19` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1948 | 1947 | `org.springframework:spring-messaging` | java | `5.3.22` | `6.2.19` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`5.2.5.RELEASE`, `5.3.22`。
- 升级候选边：`5.2.5.RELEASE`, `5.3.22`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
