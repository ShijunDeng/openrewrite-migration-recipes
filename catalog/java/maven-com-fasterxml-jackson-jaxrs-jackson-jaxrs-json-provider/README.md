# com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-fasterxml-jackson-jaxrs-jackson-jaxrs-json-provider` |
| Maven artifactId | `migration-spec-java-maven-com-fasterxml-jackson-jaxrs-jackson-jaxrs-json-provider` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` |
| Catalog canonical identity | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.21.2` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3137 | 3136 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.10.3` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3138 | 3137 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.13.5` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3139 | 3138 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.14.0-rc1` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3140 | 3139 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.14.0-rc2` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3141 | 3140 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.15.1` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3142 | 3141 | `com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider` | java | `2.15.2` | `2.21.2` | 14 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.10.3`, `2.13.5`, `2.14.0-rc1`, `2.14.0-rc2`, `2.15.1`, `2.15.2`。
- 升级候选边：`2.10.3`, `2.13.5`, `2.14.0-rc1`, `2.14.0-rc2`, `2.15.1`, `2.15.2`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3137 2.10.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3138 2.13.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3139 2.14.0-rc1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3140 2.14.0-rc2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3141 2.15.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3142 2.15.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.21.2` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
