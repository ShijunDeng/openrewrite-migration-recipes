# com.github.docker-java:docker-java 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-docker-java-docker-java` |
| Maven artifactId | `migration-spec-java-maven-com-github-docker-java-docker-java` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.docker-java:docker-java` |
| Catalog canonical identity | `com.github.docker-java:docker-java`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.3.4` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4774 | 4773 | `com.github.docker-java:docker-java` | java | `3.3.2` | `3.3.4` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4774 3.3.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.3.4` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
