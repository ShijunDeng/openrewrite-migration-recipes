# org.apache.httpcomponents.client5:httpclient5 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-httpcomponents-client5-httpclient5` |
| Maven artifactId | `migration-spec-java-maven-org-apache-httpcomponents-client5-httpclient5` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.httpcomponents.client5:httpclient5` |
| Catalog canonical identity | `org.apache.httpcomponents.client5:httpclient5`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `5.6.1` |
| Excel 迁移边 | 3 |
| 涉及微服务数 | 最大可见值 `33`；不同版本行不累加 |
| 分桶 | `B3_Minor联动` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1156 | 1155 | `org.apache.httpcomponents.client5:httpclient5` | java | `5.1.3` | `5.6.1` | 33 | B3_Minor联动 | 中 | upgrade-candidate/mark | httpcomponents联动组需同步版本，minor升级需保持一致 |
| 1157 | 1156 | `org.apache.httpcomponents.client5:httpclient5` | java | `5.2` | `5.6.1` | 33 | B3_Minor联动 | 中 | upgrade-candidate/mark | httpcomponents联动组需同步版本，minor升级需保持一致 |
| 1158 | 1157 | `org.apache.httpcomponents.client5:httpclient5` | java | `5.2.1` | `5.6.1` | 33 | B3_Minor联动 | 中 | upgrade-candidate/mark | httpcomponents联动组需同步版本，minor升级需保持一致 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`5.1.3`, `5.2`, `5.2.1`。
- 升级候选边：`5.1.3`, `5.2`, `5.2.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 依赖族对齐 / BOM / 平台 | Excel #1156 5.1.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1157 5.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1158 5.2.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `5.6.1` | httpcomponents联动组需同步版本，minor升级需保持一致 | `UNVERIFIED` | 建立联动成员图并迁移 BOM、platform、parent 或共享 property 的真实 owner；禁止只改一个叶子依赖造成二进制或运行时漂移。 |
