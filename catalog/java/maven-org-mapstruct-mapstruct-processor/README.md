# org.mapstruct:mapstruct-processor 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-mapstruct-mapstruct-processor` |
| Maven artifactId | `migration-spec-java-maven-org-mapstruct-mapstruct-processor` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.mapstruct:mapstruct-processor` |
| Catalog canonical identity | `org.mapstruct:mapstruct-processor`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.6.3` |
| Excel 迁移边 | 4 |
| 涉及微服务数 | 最大可见值 `128`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2331 | 2330 | `org.mapstruct:mapstruct-processor` | java | `1.4.2.Final` | `1.6.3` | 128 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2332 | 2331 | `org.mapstruct:mapstruct-processor` | java | `1.5.3.Final` | `1.6.3` | 128 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2333 | 2332 | `org.mapstruct:mapstruct-processor` | java | `1.5.4.Final` | `1.6.3` | 128 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2334 | 2333 | `org.mapstruct:mapstruct-processor` | java | `1.5.5.Final` | `1.6.3` | 128 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.4.2.Final`, `1.5.3.Final`, `1.5.4.Final`, `1.5.5.Final`。
- 升级候选边：`1.4.2.Final`, `1.5.3.Final`, `1.5.4.Final`, `1.5.5.Final`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2331 1.4.2.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2332 1.5.3.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2333 1.5.4.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2334 1.5.5.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.6.3` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
