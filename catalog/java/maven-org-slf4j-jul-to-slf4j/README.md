...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-slf4j-jul-to-slf4j` |
| Maven artifactId | `migration-spec-java-maven-org-slf4j-jul-to-slf4j` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.slf4j:jul-to-slf4j` |
| Catalog canonical identity | `org.slf4j:jul-to-slf4j`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.0.17` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 338 | 337 | `org.slf4j:jul-to-slf4j` | java | `1.7.30` | `2.0.17` | 18 | B8_语义不兼容 | 高 | upgrade-candidate/mark | slf4j 1.x->2.x 绑定机制变更 |
| 339 | 338 | `org.slf4j:jul-to-slf4j` | java | `1.7.32` | `2.0.17` | 18 | B8_语义不兼容 | 高 | upgrade-candidate/mark | slf4j 1.x->2.x 绑定机制变更 |
| 340 | 339 | `org.slf4j:jul-to-slf4j` | java | `1.7.36` | `2.0.17` | 18 | B8_语义不兼容 | 高 | upgrade-candidate/mark | slf4j 1.x->2.x 绑定机制变更 |
| 3009 | 3008 | `org.slf4j:jul-to-slf4j` | java | `2.0.0` | `2.0.17` | 18 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.7.30`, `1.7.32`, `1.7.36`, `2.0.0`。
- 升级候选边：`1.7.30`, `1.7.32`, `1.7.36`, `2.0.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3009 2.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.0.17` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 行为契约 / 语义 / 真实用例 | Excel #338 1.7.30 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #339 1.7.32 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #340 1.7.36 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.0.17` | slf4j 1.x->2.x 绑定机制变更 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
