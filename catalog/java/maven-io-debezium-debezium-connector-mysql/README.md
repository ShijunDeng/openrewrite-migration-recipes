# io.debezium:debezium-connector-mysql 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-debezium-debezium-connector-mysql` |
| Maven artifactId | `migration-spec-java-maven-io-debezium-debezium-connector-mysql` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.debezium:debezium-connector-mysql` |
| Catalog canonical identity | `io.debezium:debezium-connector-mysql`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.6.1.Final` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2090 | 2089 | `io.debezium:debezium-connector-mysql` | java | `1.4.0.Final` | `2.6.1.Final` | 1 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #2090 1.4.0.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.6.1.Final` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
