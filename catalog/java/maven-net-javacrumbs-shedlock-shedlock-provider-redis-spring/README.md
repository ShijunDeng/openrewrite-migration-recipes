# net.javacrumbs.shedlock:shedlock-provider-redis-spring 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-net-javacrumbs-shedlock-shedlock-provider-redis-spring` |
| Maven artifactId | `migration-spec-java-maven-net-javacrumbs-shedlock-shedlock-provider-redis-spring` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `net.javacrumbs.shedlock:shedlock-provider-redis-spring` |
| Catalog canonical identity | `net.javacrumbs.shedlock:shedlock-provider-redis-spring`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `7.2.1` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 478 | 477 | `net.javacrumbs.shedlock:shedlock-provider-redis-spring` | java | `4.33.0` | `7.2.1` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 479 | 478 | `net.javacrumbs.shedlock:shedlock-provider-redis-spring` | java | `4.41.0` | `7.2.1` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 480 | 479 | `net.javacrumbs.shedlock:shedlock-provider-redis-spring` | java | `4.44.0` | `7.2.1` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.33.0`, `4.41.0`, `4.44.0`。
- 升级候选边：`4.33.0`, `4.41.0`, `4.44.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #478 4.33.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #479 4.41.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #480 4.44.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.2.1` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
