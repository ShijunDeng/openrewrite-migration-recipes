# org.apache.xmlgraphics:xmlgraphics-commons 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-xmlgraphics-xmlgraphics-commons` |
| Maven artifactId | `migration-spec-java-maven-org-apache-xmlgraphics-xmlgraphics-commons` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.xmlgraphics:xmlgraphics-commons` |
| Catalog canonical identity | `org.apache.xmlgraphics:xmlgraphics-commons`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.1` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3808 | 3807 | `org.apache.xmlgraphics:xmlgraphics-commons` | java | `2.7` | `2.1` | 4 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |
| 3809 | 3808 | `org.apache.xmlgraphics:xmlgraphics-commons` | java | `2.8` | `2.1` | 4 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.7`, `2.8`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3808 2.7 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #3809 2.8 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `2.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
