...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-antlr-antlr4-runtime` |
| Maven artifactId | `migration-spec-java-maven-org-antlr-antlr4-runtime` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.antlr:antlr4-runtime` |
| Catalog canonical identity | `org.antlr:antlr4-runtime`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.13.2` |
| Excel 迁移边 | 8 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2894 | 2893 | `org.antlr:antlr4-runtime` | java | `4.1` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2895 | 2894 | `org.antlr:antlr4-runtime` | java | `4.10.1` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2896 | 2895 | `org.antlr:antlr4-runtime` | java | `4.11.1` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2897 | 2896 | `org.antlr:antlr4-runtime` | java | `4.12.0` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2898 | 2897 | `org.antlr:antlr4-runtime` | java | `4.13.0` | `4.13.2` | 21 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2899 | 2898 | `org.antlr:antlr4-runtime` | java | `4.7.1` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2900 | 2899 | `org.antlr:antlr4-runtime` | java | `4.9.2` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2901 | 2900 | `org.antlr:antlr4-runtime` | java | `4.9.3` | `4.13.2` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.1`, `4.10.1`, `4.11.1`, `4.12.0`, `4.13.0`, `4.7.1`, `4.9.2`, `4.9.3`。
- 升级候选边：`4.1`, `4.10.1`, `4.11.1`, `4.12.0`, `4.13.0`, `4.7.1`, `4.9.2`, `4.9.3`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2898 4.13.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.13.2` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2894 4.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2895 4.10.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2896 4.11.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2897 4.12.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2899 4.7.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2900 4.9.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2901 4.9.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.13.2` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
