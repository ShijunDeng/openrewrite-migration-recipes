# cn.aradin:springfox-swagger2 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-cn-aradin-springfox-swagger2` |
| Maven artifactId | `migration-spec-java-maven-cn-aradin-springfox-swagger2` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `cn.aradin:springfox-swagger2` |
| Catalog canonical identity | `cn.aradin:springfox-swagger2`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.1.2` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 740 | 739 | `cn.aradin:springfox-swagger2` | java | `3.0.0` | `1.1.2` | 1 | B6_Multi-major单包 | 高 | conflict/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 1989 | 1988 | `cn.aradin:springfox-swagger2` | java | `2.10.5` | `1.1.2` | 1 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |
| 1990 | 1989 | `cn.aradin:springfox-swagger2` | java | `2.6.1` | `1.1.2` | 1 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |
| 1991 | 1990 | `cn.aradin:springfox-swagger2` | java | `2.7.0` | `1.1.2` | 1 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |
| 1992 | 1991 | `cn.aradin:springfox-swagger2` | java | `2.9.2` | `1.1.2` | 1 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |
| 4316 | 4315 | `cn.aradin:springfox-swagger2` | java | `1.0.1` | `1.1.2` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4316 1.0.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.1.2` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1989 2.10.5 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #1990 2.6.1 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #1991 2.7.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #1992 2.9.2 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `1.1.2` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
| C-003 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #740 3.0.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `1.1.2` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
