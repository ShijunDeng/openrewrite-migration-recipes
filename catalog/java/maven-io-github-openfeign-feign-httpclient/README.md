# io.github.openfeign:feign-httpclient 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-github-openfeign-feign-httpclient` |
| Maven artifactId | `migration-spec-java-maven-io-github-openfeign-feign-httpclient` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.github.openfeign:feign-httpclient` |
| Catalog canonical identity | `io.github.openfeign:feign-httpclient`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `13.6` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 409 | 408 | `io.github.openfeign:feign-httpclient` | java | `10.4.0` | `13.6` | 11 | B5_Major联动 | 高 | upgrade-candidate/mark | httpcomponents联动组需同步升级，跨major有breaking change风险 |
| 410 | 409 | `io.github.openfeign:feign-httpclient` | java | `12` | `13.6` | 11 | B5_Major联动 | 高 | upgrade-candidate/mark | httpcomponents联动组需同步升级，跨major有breaking change风险 |
| 411 | 410 | `io.github.openfeign:feign-httpclient` | java | `12.1` | `13.6` | 11 | B5_Major联动 | 高 | upgrade-candidate/mark | httpcomponents联动组需同步升级，跨major有breaking change风险 |
| 412 | 411 | `io.github.openfeign:feign-httpclient` | java | `12.2` | `13.6` | 11 | B5_Major联动 | 高 | upgrade-candidate/mark | httpcomponents联动组需同步升级，跨major有breaking change风险 |
| 413 | 412 | `io.github.openfeign:feign-httpclient` | java | `12.4` | `13.6` | 11 | B5_Major联动 | 高 | upgrade-candidate/mark | httpcomponents联动组需同步升级，跨major有breaking change风险 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`10.4.0`, `12`, `12.1`, `12.2`, `12.4`。
- 升级候选边：`10.4.0`, `12`, `12.1`, `12.2`, `12.4`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 跨主版本 API + 依赖族联动 | Excel #409 10.4.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #410 12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #411 12.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #412 12.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #413 12.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `13.6` | httpcomponents联动组需同步升级，跨major有breaking change风险 | `UNVERIFIED` | 同时执行主版本兼容矩阵和联动成员图；优先迁移 BOM/平台 owner，逐成员验证二进制、配置、协议和部署兼容性。 |
