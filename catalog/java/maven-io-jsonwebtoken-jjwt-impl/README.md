# io.jsonwebtoken:jjwt-impl 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-jsonwebtoken-jjwt-impl` |
| Maven artifactId | `migration-spec-java-maven-io-jsonwebtoken-jjwt-impl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.jsonwebtoken:jjwt-impl` |
| Catalog canonical identity | `io.jsonwebtoken:jjwt-impl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `0.12.6` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2885 | 2884 | `io.jsonwebtoken:jjwt-impl` | java | `0.11.2` | `0.12.6` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2886 | 2885 | `io.jsonwebtoken:jjwt-impl` | java | `0.11.5` | `0.12.6` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2885 0.11.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2886 0.11.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `0.12.6` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
