# com.alibaba:transmittable-thread-local 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-alibaba-transmittable-thread-local` |
| Maven artifactId | `migration-spec-java-maven-com-alibaba-transmittable-thread-local` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.alibaba:transmittable-thread-local` |
| Catalog canonical identity | `com.alibaba:transmittable-thread-local`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.14.5` |
| Excel 迁移边 | 7 |
| 涉及微服务数 | 最大可见值 `57`；不同版本行不累加 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2473 | 2472 | `com.alibaba:transmittable-thread-local` | java | `2.11.4` | `2.14.5` | 57 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2474 | 2473 | `com.alibaba:transmittable-thread-local` | java | `2.12.6` | `2.14.5` | 57 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2475 | 2474 | `com.alibaba:transmittable-thread-local` | java | `2.13.2` | `2.14.5` | 57 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2476 | 2475 | `com.alibaba:transmittable-thread-local` | java | `2.14.0` | `2.14.5` | 57 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2477 | 2476 | `com.alibaba:transmittable-thread-local` | java | `2.14.2` | `2.14.5` | 57 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2478 | 2477 | `com.alibaba:transmittable-thread-local` | java | `2.14.3` | `2.14.5` | 57 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2479 | 2478 | `com.alibaba:transmittable-thread-local` | java | `2.2.0` | `2.14.5` | 57 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.11.4`, `2.12.6`, `2.13.2`, `2.14.0`, `2.14.2`, `2.14.3`, `2.2.0`。
- 升级候选边：`2.11.4`, `2.12.6`, `2.13.2`, `2.14.0`, `2.14.2`, `2.14.3`, `2.2.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2476 2.14.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2477 2.14.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2478 2.14.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.14.5` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2473 2.11.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2474 2.12.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2475 2.13.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2479 2.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.14.5` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
