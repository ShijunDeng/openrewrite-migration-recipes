# com.oracle.database.jdbc:ojdbc8 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-oracle-database-jdbc-ojdbc8` |
| Maven artifactId | `migration-spec-java-maven-com-oracle-database-jdbc-ojdbc8` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.oracle.database.jdbc:ojdbc8` |
| Catalog canonical identity | `com.oracle.database.jdbc:ojdbc8`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `23.26.1.0.0` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 356 | 355 | `com.oracle.database.jdbc:ojdbc8` | java | `19.19.0.0` | `23.26.1.0.0` | 16 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 357 | 356 | `com.oracle.database.jdbc:ojdbc8` | java | `21.9.0.0` | `23.26.1.0.0` | 16 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 3065 | 3064 | `com.oracle.database.jdbc:ojdbc8` | java | `23.2.0.0` | `23.26.1.0.0` | 16 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`19.19.0.0`, `21.9.0.0`, `23.2.0.0`。
- 升级候选边：`19.19.0.0`, `21.9.0.0`, `23.2.0.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3065 23.2.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `23.26.1.0.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #356 19.19.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #357 21.9.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `23.26.1.0.0` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
