# com.linkedin.calcite:calcite-core 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-linkedin-calcite-calcite-core` |
| Maven artifactId | `migration-spec-java-maven-com-linkedin-calcite-calcite-core` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.linkedin.calcite:calcite-core` |
| Catalog canonical identity | `com.linkedin.calcite:calcite-core`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.21.0.267` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4714 | 4713 | `com.linkedin.calcite:calcite-core` | java | `1.21.0.152` | `1.21.0.267` | 1 | B0_无需升级 | 低 | upgrade-candidate/mark | 版本相同或无法解析，可忽略 |
| 4715 | 4714 | `com.linkedin.calcite:calcite-core` | java | `1.32.0` | `1.21.0.267` | 1 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |
| 4716 | 4715 | `com.linkedin.calcite:calcite-core` | java | `1.33.0` | `1.21.0.267` | 1 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.21.0.152`, `1.32.0`, `1.33.0`。
- 升级候选边：`1.21.0.152`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`1.32.0`, `1.33.0`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4714 1.21.0.152 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.21.0.267` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4715 1.32.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #4716 1.33.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `1.21.0.267` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
