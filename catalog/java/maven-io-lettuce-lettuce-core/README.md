...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-lettuce-lettuce-core` |
| Maven artifactId | `migration-spec-java-maven-io-lettuce-lettuce-core` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.lettuce:lettuce-core` |
| Catalog canonical identity | `io.lettuce:lettuce-core`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `6.8.2.RELEASE` |
| Excel 迁移边 | 7 |
| 涉及微服务数 | 最大可见值 `43`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2564 | 2563 | `io.lettuce:lettuce-core` | java | `6.1.10.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2565 | 2564 | `io.lettuce:lettuce-core` | java | `6.1.9.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2566 | 2565 | `io.lettuce:lettuce-core` | java | `6.2.1.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2567 | 2566 | `io.lettuce:lettuce-core` | java | `6.2.3.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2568 | 2567 | `io.lettuce:lettuce-core` | java | `6.2.3.RELEASE-h1` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2569 | 2568 | `io.lettuce:lettuce-core` | java | `6.2.4.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2570 | 2569 | `io.lettuce:lettuce-core` | java | `6.2.5.RELEASE` | `6.8.2.RELEASE` | 43 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`6.1.10.RELEASE`, `6.1.9.RELEASE`, `6.2.1.RELEASE`, `6.2.3.RELEASE`, `6.2.3.RELEASE-h1`, `6.2.4.RELEASE`, `6.2.5.RELEASE`。
- 升级候选边：`6.1.10.RELEASE`, `6.1.9.RELEASE`, `6.2.1.RELEASE`, `6.2.3.RELEASE`, `6.2.3.RELEASE-h1`, `6.2.4.RELEASE`, `6.2.5.RELEASE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2564 6.1.10.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2565 6.1.9.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2566 6.2.1.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2567 6.2.3.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2568 6.2.3.RELEASE-h1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2569 6.2.4.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2570 6.2.5.RELEASE [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.8.2.RELEASE` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
