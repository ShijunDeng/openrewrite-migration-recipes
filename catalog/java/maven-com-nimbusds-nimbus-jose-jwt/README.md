# com.nimbusds:nimbus-jose-jwt 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-nimbusds-nimbus-jose-jwt` |
| Maven artifactId | `migration-spec-java-maven-com-nimbusds-nimbus-jose-jwt` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.nimbusds:nimbus-jose-jwt` |
| Catalog canonical identity | `com.nimbusds:nimbus-jose-jwt`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `10.8` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1774 | 1773 | `com.nimbusds:nimbus-jose-jwt` | java | `9.13` | `10.8` | 4 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1775 | 1774 | `com.nimbusds:nimbus-jose-jwt` | java | `9.31` | `10.8` | 4 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1776 | 1775 | `com.nimbusds:nimbus-jose-jwt` | java | `9.37` | `10.8` | 4 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`9.13`, `9.31`, `9.37`。
- 升级候选边：`9.13`, `9.31`, `9.37`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1774 9.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1775 9.31 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1776 9.37 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `10.8` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
