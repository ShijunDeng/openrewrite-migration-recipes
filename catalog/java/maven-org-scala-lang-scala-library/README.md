# org.scala-lang:scala-library 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-scala-lang-scala-library` |
| Maven artifactId | `migration-spec-java-maven-org-scala-lang-scala-library` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.scala-lang:scala-library` |
| Catalog canonical identity | `org.scala-lang:scala-library`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.12.21` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2665 | 2664 | `org.scala-lang:scala-library` | java | `2.11.12` | `2.12.21` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2666 | 2665 | `org.scala-lang:scala-library` | java | `2.12.16` | `2.12.21` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2667 | 2666 | `org.scala-lang:scala-library` | java | `2.12.17` | `2.12.21` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2668 | 2667 | `org.scala-lang:scala-library` | java | `2.12.18` | `2.12.21` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2669 | 2668 | `org.scala-lang:scala-library` | java | `2.12.8` | `2.12.21` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2670 | 2669 | `org.scala-lang:scala-library` | java | `2.13.9` | `2.12.21` | 31 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.11.12`, `2.12.16`, `2.12.17`, `2.12.18`, `2.12.8`, `2.13.9`。
- 升级候选边：`2.11.12`, `2.12.16`, `2.12.17`, `2.12.18`, `2.12.8`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`2.13.9`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2666 2.12.16 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2667 2.12.17 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2668 2.12.18 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2669 2.12.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.12.21` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2665 2.11.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2670 2.13.9 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `2.12.21` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
