...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-hankcs-hanlp` |
| Maven artifactId | `migration-spec-java-maven-com-hankcs-hanlp` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.hankcs:hanlp` |
| Catalog canonical identity | `com.hankcs:hanlp`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `portable-1.8.6` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2657 | 2656 | `com.hankcs:hanlp` | java | `portable-1.7.8` | `portable-1.8.6` | 32 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 2658 | 2657 | `com.hankcs:hanlp` | java | `portable-1.8.3` | `portable-1.8.6` | 32 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 2659 | 2658 | `com.hankcs:hanlp` | java | `portable-1.8.4` | `portable-1.8.6` | 32 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`portable-1.7.8`, `portable-1.8.3`, `portable-1.8.4`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`portable-1.7.8`, `portable-1.8.3`, `portable-1.8.4`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #2657 portable-1.7.8 [unknown/mark: 当前只做保守数字前缀比较；版本体系或制品身份未验证，禁止猜测式 AUTO。]<br>Excel #2658 portable-1.8.3 [unknown/mark: 当前只做保守数字前缀比较；版本体系或制品身份未验证，禁止猜测式 AUTO。]<br>Excel #2659 portable-1.8.4 [unknown/mark: 当前只做保守数字前缀比较；版本体系或制品身份未验证，禁止猜测式 AUTO。] → `portable-1.8.6` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
