# com.huaweicloud:esdk-obs-java 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-huaweicloud-esdk-obs-java` |
| Maven artifactId | `migration-spec-java-maven-com-huaweicloud-esdk-obs-java` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.huaweicloud:esdk-obs-java` |
| Catalog canonical identity | `com.huaweicloud:esdk-obs-java`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.25.10` |
| Excel 迁移边 | 7 |
| 涉及微服务数 | 最大可见值 `107`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2354 | 2353 | `com.huaweicloud:esdk-obs-java` | java | `3.21.12` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2355 | 2354 | `com.huaweicloud:esdk-obs-java` | java | `3.21.4.1` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2356 | 2355 | `com.huaweicloud:esdk-obs-java` | java | `3.21.8.1` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2357 | 2356 | `com.huaweicloud:esdk-obs-java` | java | `3.22.12` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2358 | 2357 | `com.huaweicloud:esdk-obs-java` | java | `3.22.3.1` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2359 | 2358 | `com.huaweicloud:esdk-obs-java` | java | `3.23.3` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2360 | 2359 | `com.huaweicloud:esdk-obs-java` | java | `3.23.3.1` | `3.25.10` | 107 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.21.12`, `3.21.4.1`, `3.21.8.1`, `3.22.12`, `3.22.3.1`, `3.23.3`, `3.23.3.1`。
- 升级候选边：`3.21.12`, `3.21.4.1`, `3.21.8.1`, `3.22.12`, `3.22.3.1`, `3.23.3`, `3.23.3.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2354 3.21.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2355 3.21.4.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2356 3.21.8.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2357 3.22.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2358 3.22.3.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2359 3.23.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2360 3.23.3.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.25.10` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
