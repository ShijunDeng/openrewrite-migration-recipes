# com.alibaba.csp:sentinel-datasource-extension 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-alibaba-csp-sentinel-datasource-extension` |
| Maven artifactId | `migration-spec-java-maven-com-alibaba-csp-sentinel-datasource-extension` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.alibaba.csp:sentinel-datasource-extension` |
| Catalog canonical identity | `com.alibaba.csp:sentinel-datasource-extension`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.8.8` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4786 | 4785 | `com.alibaba.csp:sentinel-datasource-extension` | java | `1.8.6` | `1.8.8` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4786 1.8.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.8.8` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
