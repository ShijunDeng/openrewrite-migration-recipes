...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-tk-mybatis-mapper` |
| Maven artifactId | `migration-spec-java-maven-tk-mybatis-mapper` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `tk.mybatis:mapper` |
| Catalog canonical identity | `tk.mybatis:mapper`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.3.2` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2851 | 2850 | `tk.mybatis:mapper` | java | `4.2.1` | `4.3.2` | 23 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2852 | 2851 | `tk.mybatis:mapper` | java | `4.2.3` | `4.3.2` | 23 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2851 4.2.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2852 4.2.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.3.2` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
