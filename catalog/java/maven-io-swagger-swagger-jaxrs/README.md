...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-swagger-swagger-jaxrs` |
| Maven artifactId | `migration-spec-java-maven-io-swagger-swagger-jaxrs` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.swagger:swagger-jaxrs` |
| Catalog canonical identity | `io.swagger:swagger-jaxrs`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.6.16` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4141 | 4140 | `io.swagger:swagger-jaxrs` | java | `1.6.9` | `1.6.16` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.6.9`。
- 升级候选边：`1.6.9`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4141 1.6.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.6.16` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
