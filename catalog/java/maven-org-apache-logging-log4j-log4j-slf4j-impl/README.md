# org.apache.logging.log4j:log4j-slf4j-impl 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-logging-log4j-log4j-slf4j-impl` |
| Maven artifactId | `migration-spec-java-maven-org-apache-logging-log4j-log4j-slf4j-impl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.logging.log4j:log4j-slf4j-impl` |
| Catalog canonical identity | `org.apache.logging.log4j:log4j-slf4j-impl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.25.5` |
| Excel 迁移边 | 6 |
| 涉及微服务数 | 最大可见值 `114`；不同版本行不累加 |
| 分桶 | `B3_Minor联动` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 982 | 981 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.17.0` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 983 | 982 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.17.1` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 984 | 983 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.17.2` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 985 | 984 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.18.0` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 986 | 985 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.19.0` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 987 | 986 | `org.apache.logging.log4j:log4j-slf4j-impl` | java | `2.20.0` | `2.25.5` | 114 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.17.0`, `2.17.1`, `2.17.2`, `2.18.0`, `2.19.0`, `2.20.0`。
- 升级候选边：`2.17.0`, `2.17.1`, `2.17.2`, `2.18.0`, `2.19.0`, `2.20.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 依赖族对齐 / BOM / 平台 | Excel #982 2.17.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #983 2.17.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #984 2.17.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #985 2.18.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #986 2.19.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #987 2.20.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.25.5` | log4j联动组需同步版本，minor升级需保持一致 | `UNVERIFIED` | 建立联动成员图并迁移 BOM、platform、parent 或共享 property 的真实 owner；禁止只改一个叶子依赖造成二进制或运行时漂移。 |
