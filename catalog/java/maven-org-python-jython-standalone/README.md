# org.python:jython-standalone 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-python-jython-standalone` |
| Maven artifactId | `migration-spec-java-maven-org-python-jython-standalone` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.python:jython-standalone` |
| Catalog canonical identity | `org.python:jython-standalone`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.7.4` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4226 | 4225 | `org.python:jython-standalone` | java | `2.7.3` | `2.7.4` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4226 2.7.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.7.4` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
