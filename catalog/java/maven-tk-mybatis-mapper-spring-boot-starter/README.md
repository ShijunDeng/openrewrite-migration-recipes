# tk.mybatis:mapper-spring-boot-starter 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-tk-mybatis-mapper-spring-boot-starter` |
| Maven artifactId | `migration-spec-java-maven-tk-mybatis-mapper-spring-boot-starter` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `tk.mybatis:mapper-spring-boot-starter` |
| Catalog canonical identity | `tk.mybatis:mapper-spring-boot-starter`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `6.0.0` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 443 | 442 | `tk.mybatis:mapper-spring-boot-starter` | java | `4.2.1` | `6.0.0` | 10 | B8_语义不兼容 | 高 | upgrade-candidate/mark | MyBatis跨多个大版本(4.2.1->6.0.0)，配置方式/注解API变更 |
| 444 | 443 | `tk.mybatis:mapper-spring-boot-starter` | java | `4.2.3` | `6.0.0` | 10 | B8_语义不兼容 | 高 | upgrade-candidate/mark | MyBatis跨多个大版本(4.2.3->6.0.0)，配置方式/注解API变更 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.2.1`, `4.2.3`。
- 升级候选边：`4.2.1`, `4.2.3`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 行为契约 / 语义 / 真实用例 | Excel #443 4.2.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.0.0` | MyBatis跨多个大版本(4.2.1->6.0.0)，配置方式/注解API变更 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-002 | 行为契约 / 语义 / 真实用例 | Excel #444 4.2.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.0.0` | MyBatis跨多个大版本(4.2.3->6.0.0)，配置方式/注解API变更 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
