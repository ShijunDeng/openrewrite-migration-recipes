# org.apache.httpcomponents.core5:httpcore5-h2 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-httpcomponents-core5-httpcore5-h2` |
| Maven artifactId | `migration-spec-java-maven-org-apache-httpcomponents-core5-httpcore5-h2` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.httpcomponents.core5:httpcore5-h2` |
| Catalog canonical identity | `org.apache.httpcomponents.core5:httpcore5-h2`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `5.4` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1836 | 1835 | `org.apache.httpcomponents.core5:httpcore5-h2` | java | `5.2` | `5.4` | 3 | B3_Minor联动 | 中 | upgrade-candidate/mark | httpcomponents联动组需同步版本，minor升级需保持一致 |
| 1837 | 1836 | `org.apache.httpcomponents.core5:httpcore5-h2` | java | `5.2.2` | `5.4` | 3 | B3_Minor联动 | 中 | upgrade-candidate/mark | httpcomponents联动组需同步版本，minor升级需保持一致 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`5.2`, `5.2.2`。
- 升级候选边：`5.2`, `5.2.2`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 依赖族对齐 / BOM / 平台 | Excel #1836 5.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1837 5.2.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `5.4` | httpcomponents联动组需同步版本，minor升级需保持一致 | `UNVERIFIED` | 建立联动成员图并迁移 BOM、platform、parent 或共享 property 的真实 owner；禁止只改一个叶子依赖造成二进制或运行时漂移。 |
