...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-deepoove-poi-tl` |
| Maven artifactId | `migration-spec-java-maven-com-deepoove-poi-tl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.deepoove:poi-tl` |
| Catalog canonical identity | `com.deepoove:poi-tl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.12.2` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3890 | 3889 | `com.deepoove:poi-tl` | java | `1.12.1` | `1.12.2` | 4 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.12.1`。
- 升级候选边：`1.12.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3890 1.12.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.12.2` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
