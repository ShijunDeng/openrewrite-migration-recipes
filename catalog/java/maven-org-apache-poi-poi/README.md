...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-poi-poi` |
| Maven artifactId | `migration-spec-java-maven-org-apache-poi-poi` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.poi:poi` |
| Catalog canonical identity | `org.apache.poi:poi`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `5.4.1` |
| Excel 迁移边 | 2 |
| 涉及微服务数 | 最大可见值 `77`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2414 | 2413 | `org.apache.poi:poi` | java | `5.2.2` | `5.4.1` | 77 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2415 | 2414 | `org.apache.poi:poi` | java | `5.2.3` | `5.4.1` | 77 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2414 5.2.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2415 5.2.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `5.4.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
