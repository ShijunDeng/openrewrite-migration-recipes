# io.serverlessworkflow:serverlessworkflow-util 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-serverlessworkflow-serverlessworkflow-util` |
| Maven artifactId | `migration-spec-java-maven-io-serverlessworkflow-serverlessworkflow-util` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.serverlessworkflow:serverlessworkflow-util` |
| Catalog canonical identity | `io.serverlessworkflow:serverlessworkflow-util`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.0.5.Final` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4429 | 4428 | `io.serverlessworkflow:serverlessworkflow-util` | java | `4.0.3.Final` | `4.0.5.Final` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4429 4.0.3.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.5.Final` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
