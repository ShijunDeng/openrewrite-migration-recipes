# org.apache.logging.log4j:log4j-jul 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-logging-log4j-log4j-jul` |
| Maven artifactId | `migration-spec-java-maven-org-apache-logging-log4j-log4j-jul` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.logging.log4j:log4j-jul` |
| Catalog canonical identity | `org.apache.logging.log4j:log4j-jul`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.25.5` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2719 | 2718 | `org.apache.logging.log4j:log4j-jul` | java | `2.17.0` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2720 | 2719 | `org.apache.logging.log4j:log4j-jul` | java | `2.17.1` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2721 | 2720 | `org.apache.logging.log4j:log4j-jul` | java | `2.17.2` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2722 | 2721 | `org.apache.logging.log4j:log4j-jul` | java | `2.18.0` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2723 | 2722 | `org.apache.logging.log4j:log4j-jul` | java | `2.19.0` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2724 | 2723 | `org.apache.logging.log4j:log4j-jul` | java | `2.20.0` | `2.25.5` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2719 2.17.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2720 2.17.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2721 2.17.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2722 2.18.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2723 2.19.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2724 2.20.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.25.5` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
