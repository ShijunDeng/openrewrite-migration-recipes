# com.sun.xml.bind:jaxb-impl 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-sun-xml-bind-jaxb-impl` |
| Maven artifactId | `migration-spec-java-maven-com-sun-xml-bind-jaxb-impl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.sun.xml.bind:jaxb-impl` |
| Catalog canonical identity | `com.sun.xml.bind:jaxb-impl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.0.6` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 382 | 381 | `com.sun.xml.bind:jaxb-impl` | java | `2.3.8` | `4.0.6` | 14 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 3190 | 3189 | `com.sun.xml.bind:jaxb-impl` | java | `4.0.2` | `4.0.6` | 14 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3191 | 3190 | `com.sun.xml.bind:jaxb-impl` | java | `4.0.3` | `4.0.6` | 14 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.3.8`, `4.0.2`, `4.0.3`。
- 升级候选边：`2.3.8`, `4.0.2`, `4.0.3`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3190 4.0.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3191 4.0.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.6` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #382 2.3.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.6` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
