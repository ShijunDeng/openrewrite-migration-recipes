# jakarta.xml.ws:jakarta.xml.ws-api 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-jakarta-xml-ws-jakarta-xml-ws-api` |
| Maven artifactId | `migration-spec-java-maven-jakarta-xml-ws-jakarta-xml-ws-api` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `jakarta.xml.ws:jakarta.xml.ws-api` |
| Catalog canonical identity | `jakarta.xml.ws:jakarta.xml.ws-api`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.0.3` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 692 | 691 | `jakarta.xml.ws:jakarta.xml.ws-api` | java | `2.3.3` | `4.0.3` | 2 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 4162 | 4161 | `jakarta.xml.ws:jakarta.xml.ws-api` | java | `4.0.0` | `4.0.3` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.3.3`, `4.0.0`。
- 升级候选边：`2.3.3`, `4.0.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4162 4.0.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #692 2.3.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
