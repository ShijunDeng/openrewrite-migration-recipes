# com.github.ulisesbocchio:jasypt-spring-boot-starter 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-ulisesbocchio-jasypt-spring-boot-starter` |
| Maven artifactId | `migration-spec-java-maven-com-github-ulisesbocchio-jasypt-spring-boot-starter` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` |
| Catalog canonical identity | `com.github.ulisesbocchio:jasypt-spring-boot-starter`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.0.3` |
| Excel 迁移边 | 5 |
| 涉及微服务数 | 最大可见值 `131`；不同版本行不累加 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 140 | 139 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` | java | `2.1.1` | `4.0.3` | 131 | B8_语义不兼容 | 高 | upgrade-candidate/mark | jasypt跨多个大版本(2.1.1->4.0.3)，加密配置方式变更 |
| 141 | 140 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` | java | `2.1.2` | `4.0.3` | 131 | B8_语义不兼容 | 高 | upgrade-candidate/mark | jasypt跨多个大版本(2.1.2->4.0.3)，加密配置方式变更 |
| 958 | 957 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` | java | `3.0.3` | `4.0.3` | 131 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 959 | 958 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` | java | `3.0.4` | `4.0.3` | 131 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 960 | 959 | `com.github.ulisesbocchio:jasypt-spring-boot-starter` | java | `3.0.5` | `4.0.3` | 131 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.1.1`, `2.1.2`, `3.0.3`, `3.0.4`, `3.0.5`。
- 升级候选边：`2.1.1`, `2.1.2`, `3.0.3`, `3.0.4`, `3.0.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #958 3.0.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #959 3.0.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #960 3.0.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
| C-002 | 行为契约 / 语义 / 真实用例 | Excel #140 2.1.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | jasypt跨多个大版本(2.1.1->4.0.3)，加密配置方式变更 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-003 | 行为契约 / 语义 / 真实用例 | Excel #141 2.1.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | jasypt跨多个大版本(2.1.2->4.0.3)，加密配置方式变更 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
