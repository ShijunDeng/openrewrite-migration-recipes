# com.huaweicloud.sdk:huaweicloud-sdk-iam 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-huaweicloud-sdk-huaweicloud-sdk-iam` |
| Maven artifactId | `migration-spec-java-maven-com-huaweicloud-sdk-huaweicloud-sdk-iam` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.huaweicloud.sdk:huaweicloud-sdk-iam` |
| Catalog canonical identity | `com.huaweicloud.sdk:huaweicloud-sdk-iam`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.1.177` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3468 | 3467 | `com.huaweicloud.sdk:huaweicloud-sdk-iam` | java | `3.1.28` | `3.1.177` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3469 | 3468 | `com.huaweicloud.sdk:huaweicloud-sdk-iam` | java | `3.1.47` | `3.1.177` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3470 | 3469 | `com.huaweicloud.sdk:huaweicloud-sdk-iam` | java | `3.1.48` | `3.1.177` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3471 | 3470 | `com.huaweicloud.sdk:huaweicloud-sdk-iam` | java | `3.1.6` | `3.1.177` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.1.28`, `3.1.47`, `3.1.48`, `3.1.6`。
- 升级候选边：`3.1.28`, `3.1.47`, `3.1.48`, `3.1.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3468 3.1.28 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3469 3.1.47 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3470 3.1.48 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3471 3.1.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.1.177` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
