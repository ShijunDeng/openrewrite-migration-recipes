# org.elasticsearch.client:elasticsearch-rest-client 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-elasticsearch-client-elasticsearch-rest-client` |
| Maven artifactId | `migration-spec-java-maven-org-elasticsearch-client-elasticsearch-rest-client` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.elasticsearch.client:elasticsearch-rest-client` |
| Catalog canonical identity | `org.elasticsearch.client:elasticsearch-rest-client`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `8.18.3` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1566 | 1565 | `org.elasticsearch.client:elasticsearch-rest-client` | java | `7.10.2` | `8.18.3` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1567 | 1566 | `org.elasticsearch.client:elasticsearch-rest-client` | java | `7.10.2-hw-ei-312056` | `8.18.3` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 3363 | 3362 | `org.elasticsearch.client:elasticsearch-rest-client` | java | `8.7.1` | `8.18.3` | 9 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`7.10.2`, `7.10.2-hw-ei-312056`, `8.7.1`。
- 升级候选边：`7.10.2`, `7.10.2-hw-ei-312056`, `8.7.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3363 8.7.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `8.18.3` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1566 7.10.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1567 7.10.2-hw-ei-312056 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `8.18.3` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
