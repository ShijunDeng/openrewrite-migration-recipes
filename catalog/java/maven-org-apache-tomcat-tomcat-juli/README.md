# org.apache.tomcat:tomcat-juli 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-tomcat-tomcat-juli` |
| Maven artifactId | `migration-spec-java-maven-org-apache-tomcat-tomcat-juli` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.tomcat:tomcat-juli` |
| Catalog canonical identity | `org.apache.tomcat:tomcat-juli`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `10.1.54` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1891 | 1890 | `org.apache.tomcat:tomcat-juli` | java | `9.0.65` | `10.1.54` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 4127 | 4126 | `org.apache.tomcat:tomcat-juli` | java | `10.1.8` | `10.1.54` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`10.1.8`, `9.0.65`。
- 升级候选边：`10.1.8`, `9.0.65`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4127 10.1.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `10.1.54` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1891 9.0.65 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `10.1.54` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
