# org.mongodb:mongodb-driver-sync 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-mongodb-mongodb-driver-sync` |
| Maven artifactId | `migration-spec-java-maven-org-mongodb-mongodb-driver-sync` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.mongodb:mongodb-driver-sync` |
| Catalog canonical identity | `org.mongodb:mongodb-driver-sync`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `5.5.2` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1546 | 1545 | `org.mongodb:mongodb-driver-sync` | java | `4.10.2` | `5.5.2` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1547 | 1546 | `org.mongodb:mongodb-driver-sync` | java | `4.6.1` | `5.5.2` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1548 | 1547 | `org.mongodb:mongodb-driver-sync` | java | `4.7.2` | `5.5.2` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1549 | 1548 | `org.mongodb:mongodb-driver-sync` | java | `4.8.0` | `5.5.2` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1550 | 1549 | `org.mongodb:mongodb-driver-sync` | java | `4.9.1` | `5.5.2` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.10.2`, `4.6.1`, `4.7.2`, `4.8.0`, `4.9.1`。
- 升级候选边：`4.10.2`, `4.6.1`, `4.7.2`, `4.8.0`, `4.9.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1546 4.10.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1547 4.6.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1548 4.7.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1549 4.8.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1550 4.9.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `5.5.2` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
