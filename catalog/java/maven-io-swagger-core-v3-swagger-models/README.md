# io.swagger.core.v3:swagger-models 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-swagger-core-v3-swagger-models` |
| Maven artifactId | `migration-spec-java-maven-io-swagger-core-v3-swagger-models` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.swagger.core.v3:swagger-models` |
| Catalog canonical identity | `io.swagger.core.v3:swagger-models`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.2.38` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1895 | 1894 | `io.swagger.core.v3:swagger-models` | java | `1.5.22` | `2.2.38` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1896 | 1895 | `io.swagger.core.v3:swagger-models` | java | `1.6.5` | `2.2.38` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1897 | 1896 | `io.swagger.core.v3:swagger-models` | java | `1.6.6` | `2.2.38` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1898 | 1897 | `io.swagger.core.v3:swagger-models` | java | `1.6.9` | `2.2.38` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 4142 | 4141 | `io.swagger.core.v3:swagger-models` | java | `2.2.0` | `2.2.38` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 4143 | 4142 | `io.swagger.core.v3:swagger-models` | java | `2.2.10` | `2.2.38` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.5.22`, `1.6.5`, `1.6.6`, `1.6.9`, `2.2.0`, `2.2.10`。
- 升级候选边：`1.5.22`, `1.6.5`, `1.6.6`, `1.6.9`, `2.2.0`, `2.2.10`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4142 2.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #4143 2.2.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.2.38` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1895 1.5.22 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1896 1.6.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1897 1.6.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1898 1.6.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.2.38` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
