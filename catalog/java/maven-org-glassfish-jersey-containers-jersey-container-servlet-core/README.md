# org.glassfish.jersey.containers:jersey-container-servlet-core 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-glassfish-jersey-containers-jersey-container-servlet-core` |
| Maven artifactId | `migration-spec-java-maven-org-glassfish-jersey-containers-jersey-container-servlet-core` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.glassfish.jersey.containers:jersey-container-servlet-core` |
| Catalog canonical identity | `org.glassfish.jersey.containers:jersey-container-servlet-core`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.1.12` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1570 | 1569 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.31` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1571 | 1570 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.34` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1572 | 1571 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.36` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1573 | 1572 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.37` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1574 | 1573 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.39.1` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1575 | 1574 | `org.glassfish.jersey.containers:jersey-container-servlet-core` | java | `2.4` | `3.1.12` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.31`, `2.34`, `2.36`, `2.37`, `2.39.1`, `2.4`。
- 升级候选边：`2.31`, `2.34`, `2.36`, `2.37`, `2.39.1`, `2.4`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1570 2.31 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1571 2.34 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1572 2.36 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1573 2.37 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1574 2.39.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1575 2.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.1.12` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
