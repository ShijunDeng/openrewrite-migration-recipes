# org.springframework.boot:spring-boot-starter-validation 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-springframework-boot-spring-boot-starter-validation` |
| Maven artifactId | `migration-spec-java-maven-org-springframework-boot-spring-boot-starter-validation` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.springframework.boot:spring-boot-starter-validation` |
| Catalog canonical identity | `org.springframework.boot:spring-boot-starter-validation`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.5.15` |
| Excel 迁移边 | 6 |
| 涉及微服务数 | 最大可见值 `16`；不同版本行不累加 |
| 分桶 | `B4_Major单包` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1380 | 1379 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.6.6` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1381 | 1380 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.7.0` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1382 | 1381 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.7.10` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1383 | 1382 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.7.12` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1384 | 1383 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.7.4` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1385 | 1384 | `org.springframework.boot:spring-boot-starter-validation` | java | `2.7.9` | `3.5.15` | 16 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.6.6`, `2.7.0`, `2.7.10`, `2.7.12`, `2.7.4`, `2.7.9`。
- 升级候选边：`2.6.6`, `2.7.0`, `2.7.10`, `2.7.12`, `2.7.4`, `2.7.9`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1380 2.6.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1381 2.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1382 2.7.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1383 2.7.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1384 2.7.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1385 2.7.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.5.15` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
