# net.javacrumbs.shedlock:shedlock-spring 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-net-javacrumbs-shedlock-shedlock-spring` |
| Maven artifactId | `migration-spec-java-maven-net-javacrumbs-shedlock-shedlock-spring` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `net.javacrumbs.shedlock:shedlock-spring` |
| Catalog canonical identity | `net.javacrumbs.shedlock:shedlock-spring`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `7.2.1` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 232 | 231 | `net.javacrumbs.shedlock:shedlock-spring` | java | `2.2.0` | `7.2.1` | 37 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 233 | 232 | `net.javacrumbs.shedlock:shedlock-spring` | java | `4.29.0` | `7.2.1` | 37 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 234 | 233 | `net.javacrumbs.shedlock:shedlock-spring` | java | `4.33.0` | `7.2.1` | 37 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 235 | 234 | `net.javacrumbs.shedlock:shedlock-spring` | java | `4.41.0` | `7.2.1` | 37 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 236 | 235 | `net.javacrumbs.shedlock:shedlock-spring` | java | `4.44.0` | `7.2.1` | 37 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.2.0`, `4.29.0`, `4.33.0`, `4.41.0`, `4.44.0`。
- 升级候选边：`2.2.0`, `4.29.0`, `4.33.0`, `4.41.0`, `4.44.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #232 2.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #233 4.29.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #234 4.33.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #235 4.41.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #236 4.44.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.2.1` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
