...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-h2database-h2` |
| Maven artifactId | `migration-spec-java-maven-com-h2database-h2` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.h2database:h2` |
| Catalog canonical identity | `com.h2database:h2`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.4.240` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3265 | 3264 | `com.h2database:h2` | java | `2.1.210` | `2.4.240` | 12 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3266 | 3265 | `com.h2database:h2` | java | `2.2.220` | `2.4.240` | 12 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.1.210`, `2.2.220`。
- 升级候选边：`2.1.210`, `2.2.220`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3265 2.1.210 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3266 2.2.220 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.4.240` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
