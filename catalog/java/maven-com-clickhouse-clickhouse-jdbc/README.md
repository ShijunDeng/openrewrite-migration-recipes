# com.clickhouse:clickhouse-jdbc 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-clickhouse-clickhouse-jdbc` |
| Maven artifactId | `migration-spec-java-maven-com-clickhouse-clickhouse-jdbc` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.clickhouse:clickhouse-jdbc` |
| Catalog canonical identity | `com.clickhouse:clickhouse-jdbc`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `0.7.1-patch1` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2819 | 2818 | `com.clickhouse:clickhouse-jdbc` | java | `0.2.6` | `0.7.1-patch1` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2820 | 2819 | `com.clickhouse:clickhouse-jdbc` | java | `0.3.2` | `0.7.1-patch1` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2821 | 2820 | `com.clickhouse:clickhouse-jdbc` | java | `0.4.2` | `0.7.1-patch1` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2822 | 2821 | `com.clickhouse:clickhouse-jdbc` | java | `0.4.6` | `0.7.1-patch1` | 24 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2819 0.2.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2820 0.3.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2821 0.4.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2822 0.4.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `0.7.1-patch1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
