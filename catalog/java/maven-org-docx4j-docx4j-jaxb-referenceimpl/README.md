# org.docx4j:docx4j-JAXB-ReferenceImpl 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-docx4j-docx4j-jaxb-referenceimpl` |
| Maven artifactId | `migration-spec-java-maven-org-docx4j-docx4j-jaxb-referenceimpl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.docx4j:docx4j-JAXB-ReferenceImpl` |
| Catalog canonical identity | `org.docx4j:docx4j-JAXB-ReferenceImpl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `11.5.3` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 647 | 646 | `org.docx4j:docx4j-JAXB-ReferenceImpl` | java | `8.3.9` | `11.5.3` | 3 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 4050 | 4049 | `org.docx4j:docx4j-JAXB-ReferenceImpl` | java | `11.4.8` | `11.5.3` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`11.4.8`, `8.3.9`。
- 升级候选边：`11.4.8`, `8.3.9`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4050 11.4.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `11.5.3` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #647 8.3.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `11.5.3` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
