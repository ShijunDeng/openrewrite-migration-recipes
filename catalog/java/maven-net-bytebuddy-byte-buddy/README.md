...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-net-bytebuddy-byte-buddy` |
| Maven artifactId | `migration-spec-java-maven-net-bytebuddy-byte-buddy` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `net.bytebuddy:byte-buddy` |
| Catalog canonical identity | `net.bytebuddy:byte-buddy`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.18.4` |
| Excel 迁移边 | 8 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2621 | 2620 | `net.bytebuddy:byte-buddy` | java | `1.11.22` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2622 | 2621 | `net.bytebuddy:byte-buddy` | java | `1.12.10` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2623 | 2622 | `net.bytebuddy:byte-buddy` | java | `1.12.13` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2624 | 2623 | `net.bytebuddy:byte-buddy` | java | `1.12.22` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2625 | 2624 | `net.bytebuddy:byte-buddy` | java | `1.12.23` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2626 | 2625 | `net.bytebuddy:byte-buddy` | java | `1.14.0` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2627 | 2626 | `net.bytebuddy:byte-buddy` | java | `1.14.5` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2628 | 2627 | `net.bytebuddy:byte-buddy` | java | `1.8.12` | `1.18.4` | 35 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.11.22`, `1.12.10`, `1.12.13`, `1.12.22`, `1.12.23`, `1.14.0`, `1.14.5`, `1.8.12`。
- 升级候选边：`1.11.22`, `1.12.10`, `1.12.13`, `1.12.22`, `1.12.23`, `1.14.0`, `1.14.5`, `1.8.12`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2621 1.11.22 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2622 1.12.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2623 1.12.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2624 1.12.22 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2625 1.12.23 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2626 1.14.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2627 1.14.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2628 1.8.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.18.4` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
