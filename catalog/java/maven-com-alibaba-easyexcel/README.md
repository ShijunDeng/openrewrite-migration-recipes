...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-alibaba-easyexcel` |
| Maven artifactId | `migration-spec-java-maven-com-alibaba-easyexcel` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.alibaba:easyexcel` |
| Catalog canonical identity | `com.alibaba:easyexcel`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.0.3` |
| Excel 迁移边 | 7 |
| 涉及微服务数 | 最大可见值 `59`；不同版本行不累加 |
| 分桶 | `B4_Major单包` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1048 | 1047 | `com.alibaba:easyexcel` | java | `3.0.5` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1049 | 1048 | `com.alibaba:easyexcel` | java | `3.1.2` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1050 | 1049 | `com.alibaba:easyexcel` | java | `3.1.3` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1051 | 1050 | `com.alibaba:easyexcel` | java | `3.1.4` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1052 | 1051 | `com.alibaba:easyexcel` | java | `3.2.1` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1053 | 1052 | `com.alibaba:easyexcel` | java | `3.3.1` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1054 | 1053 | `com.alibaba:easyexcel` | java | `3.3.2` | `4.0.3` | 59 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.0.5`, `3.1.2`, `3.1.3`, `3.1.4`, `3.2.1`, `3.3.1`, `3.3.2`。
- 升级候选边：`3.0.5`, `3.1.2`, `3.1.3`, `3.1.4`, `3.2.1`, `3.3.1`, `3.3.2`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1048 3.0.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1049 3.1.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1050 3.1.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1051 3.1.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1052 3.2.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1053 3.3.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1054 3.3.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.0.3` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
