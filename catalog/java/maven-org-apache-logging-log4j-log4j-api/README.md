# org.apache.logging.log4j:log4j-api 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-logging-log4j-log4j-api` |
| Maven artifactId | `migration-spec-java-maven-org-apache-logging-log4j-log4j-api` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.logging.log4j:log4j-api` |
| Catalog canonical identity | `org.apache.logging.log4j:log4j-api`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.25.5` |
| Excel 迁移边 | 8 |
| 涉及微服务数 | 最大可见值 `158`；不同版本行不累加 |
| 分桶 | `B3_Minor联动` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 950 | 949 | `org.apache.logging.log4j:log4j-api` | java | `2.13.2` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 951 | 950 | `org.apache.logging.log4j:log4j-api` | java | `2.13.3` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 952 | 951 | `org.apache.logging.log4j:log4j-api` | java | `2.17.0` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 953 | 952 | `org.apache.logging.log4j:log4j-api` | java | `2.17.1` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 954 | 953 | `org.apache.logging.log4j:log4j-api` | java | `2.17.2` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 955 | 954 | `org.apache.logging.log4j:log4j-api` | java | `2.18.0` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 956 | 955 | `org.apache.logging.log4j:log4j-api` | java | `2.19.0` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |
| 957 | 956 | `org.apache.logging.log4j:log4j-api` | java | `2.20.0` | `2.25.5` | 158 | B3_Minor联动 | 中 | upgrade-candidate/mark | log4j联动组需同步版本，minor升级需保持一致 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.13.2`, `2.13.3`, `2.17.0`, `2.17.1`, `2.17.2`, `2.18.0`, `2.19.0`, `2.20.0`。
- 升级候选边：`2.13.2`, `2.13.3`, `2.17.0`, `2.17.1`, `2.17.2`, `2.18.0`, `2.19.0`, `2.20.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 依赖族对齐 / BOM / 平台 | Excel #950 2.13.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #951 2.13.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #952 2.17.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #953 2.17.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #954 2.17.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #955 2.18.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #956 2.19.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #957 2.20.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.25.5` | log4j联动组需同步版本，minor升级需保持一致 | `UNVERIFIED` | 建立联动成员图并迁移 BOM、platform、parent 或共享 property 的真实 owner；禁止只改一个叶子依赖造成二进制或运行时漂移。 |
