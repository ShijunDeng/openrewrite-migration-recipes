# org.apache.cxf:cxf-rt-frontend-jaxrs 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-cxf-cxf-rt-frontend-jaxrs` |
| Maven artifactId | `migration-spec-java-maven-org-apache-cxf-cxf-rt-frontend-jaxrs` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.cxf:cxf-rt-frontend-jaxrs` |
| Catalog canonical identity | `org.apache.cxf:cxf-rt-frontend-jaxrs`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.1.7` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1971 | 1970 | `org.apache.cxf:cxf-rt-frontend-jaxrs` | java | `3.5.5` | `4.1.7` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1972 | 1971 | `org.apache.cxf:cxf-rt-frontend-jaxrs` | java | `3.5.6` | `4.1.7` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.5.5`, `3.5.6`。
- 升级候选边：`3.5.5`, `3.5.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1971 3.5.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1972 3.5.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.1.7` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
