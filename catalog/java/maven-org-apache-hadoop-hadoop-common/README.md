# org.apache.hadoop:hadoop-common 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-hadoop-hadoop-common` |
| Maven artifactId | `migration-spec-java-maven-org-apache-hadoop-hadoop-common` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.hadoop:hadoop-common` |
| Catalog canonical identity | `org.apache.hadoop:hadoop-common`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.5.0` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2873 | 2872 | `org.apache.hadoop:hadoop-common` | java | `3.1.1-hw-ei-312005` | `3.5.0` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2874 | 2873 | `org.apache.hadoop:hadoop-common` | java | `3.3.5` | `3.5.0` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2875 | 2874 | `org.apache.hadoop:hadoop-common` | java | `3.3.6` | `3.5.0` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2876 | 2875 | `org.apache.hadoop:hadoop-common` | java | `3.6.3-hw-ei-311001` | `3.5.0` | 22 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.1.1-hw-ei-312005`, `3.3.5`, `3.3.6`, `3.6.3-hw-ei-311001`。
- 升级候选边：`3.1.1-hw-ei-312005`, `3.3.5`, `3.3.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`3.6.3-hw-ei-311001`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2873 3.1.1-hw-ei-312005 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2874 3.3.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2875 3.3.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2876 3.6.3-hw-ei-311001 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `3.5.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
