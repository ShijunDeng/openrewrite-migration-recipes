# org.apache.skywalking:apm-toolkit-log4j-2.x 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-skywalking-apm-toolkit-log4j-2-x` |
| Maven artifactId | `migration-spec-java-maven-org-apache-skywalking-apm-toolkit-log4j-2-x` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.skywalking:apm-toolkit-log4j-2.x` |
| Catalog canonical identity | `org.apache.skywalking:apm-toolkit-log4j-2.x`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `9.4.0` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1542 | 1541 | `org.apache.skywalking:apm-toolkit-log4j-2.x` | java | `8.13.0` | `9.4.0` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1543 | 1542 | `org.apache.skywalking:apm-toolkit-log4j-2.x` | java | `8.16.0` | `9.4.0` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1544 | 1543 | `org.apache.skywalking:apm-toolkit-log4j-2.x` | java | `8.4.0-h1.cbu.skywalking.prod.r4` | `9.4.0` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1545 | 1544 | `org.apache.skywalking:apm-toolkit-log4j-2.x` | java | `8.4.0-hw-3.0.5` | `9.4.0` | 10 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`8.13.0`, `8.16.0`, `8.4.0-h1.cbu.skywalking.prod.r4`, `8.4.0-hw-3.0.5`。
- 升级候选边：`8.13.0`, `8.16.0`, `8.4.0-h1.cbu.skywalking.prod.r4`, `8.4.0-hw-3.0.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1542 8.13.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1543 8.16.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1544 8.4.0-h1.cbu.skywalking.prod.r4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1545 8.4.0-hw-3.0.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `9.4.0` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
