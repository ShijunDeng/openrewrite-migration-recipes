# io.kubernetes:client-java 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-kubernetes-client-java` |
| Maven artifactId | `migration-spec-java-maven-io-kubernetes-client-java` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.kubernetes:client-java` |
| Catalog canonical identity | `io.kubernetes:client-java`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `25.0.0-legacy` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 242 | 241 | `io.kubernetes:client-java` | java | `11.0.2` | `25.0.0-legacy` | 32 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 243 | 242 | `io.kubernetes:client-java` | java | `16.0.2` | `25.0.0-legacy` | 32 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 244 | 243 | `io.kubernetes:client-java` | java | `16.0.3` | `25.0.0-legacy` | 32 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 245 | 244 | `io.kubernetes:client-java` | java | `17.0.2` | `25.0.0-legacy` | 32 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 246 | 245 | `io.kubernetes:client-java` | java | `18.0.1` | `25.0.0-legacy` | 32 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`11.0.2`, `16.0.2`, `16.0.3`, `17.0.2`, `18.0.1`。
- 升级候选边：`11.0.2`, `16.0.2`, `16.0.3`, `17.0.2`, `18.0.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #242 11.0.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #243 16.0.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #244 16.0.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #245 17.0.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #246 18.0.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `25.0.0-legacy` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
