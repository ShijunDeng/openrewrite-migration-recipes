# net.bytebuddy:byte-buddy-agent 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-net-bytebuddy-byte-buddy-agent` |
| Maven artifactId | `migration-spec-java-maven-net-bytebuddy-byte-buddy-agent` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `net.bytebuddy:byte-buddy-agent` |
| Catalog canonical identity | `net.bytebuddy:byte-buddy-agent`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.17.8` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3586 | 3585 | `net.bytebuddy:byte-buddy-agent` | java | `1.11.22` | `1.17.8` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3587 | 3586 | `net.bytebuddy:byte-buddy-agent` | java | `1.12.10` | `1.17.8` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3588 | 3587 | `net.bytebuddy:byte-buddy-agent` | java | `1.12.13` | `1.17.8` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3589 | 3588 | `net.bytebuddy:byte-buddy-agent` | java | `1.12.23` | `1.17.8` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.11.22`, `1.12.10`, `1.12.13`, `1.12.23`。
- 升级候选边：`1.11.22`, `1.12.10`, `1.12.13`, `1.12.23`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3586 1.11.22 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3587 1.12.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3588 1.12.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3589 1.12.23 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.17.8` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
