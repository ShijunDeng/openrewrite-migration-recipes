# org.apache.calcite:calcite-core 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-calcite-calcite-core` |
| Maven artifactId | `migration-spec-java-maven-org-apache-calcite-calcite-core` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.calcite:calcite-core` |
| Catalog canonical identity | `org.apache.calcite:calcite-core`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.21.0.267` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3353 | 3352 | `org.apache.calcite:calcite-core` | java | `1.21.0.152` | `1.21.0.267` | 10 | B0_无需升级 | 低 | upgrade-candidate/mark | 版本相同或无法解析，可忽略 |
| 3354 | 3353 | `org.apache.calcite:calcite-core` | java | `1.32.0` | `1.21.0.267` | 10 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |
| 3355 | 3354 | `org.apache.calcite:calcite-core` | java | `1.33.0` | `1.21.0.267` | 10 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3353 1.21.0.152 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.21.0.267` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3354 1.32.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。]<br>Excel #3355 1.33.0 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `1.21.0.267` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
