# com.ctrip.framework.apollo:apollo-client 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-ctrip-framework-apollo-apollo-client` |
| Maven artifactId | `migration-spec-java-maven-com-ctrip-framework-apollo-apollo-client` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.ctrip.framework.apollo:apollo-client` |
| Catalog canonical identity | `com.ctrip.framework.apollo:apollo-client`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.4.0` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1326 | 1325 | `com.ctrip.framework.apollo:apollo-client` | java | `1.3.0` | `2.4.0` | 20 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1327 | 1326 | `com.ctrip.framework.apollo:apollo-client` | java | `1.4.0` | `2.4.0` | 20 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1328 | 1327 | `com.ctrip.framework.apollo:apollo-client` | java | `1.5.1` | `2.4.0` | 20 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1329 | 1328 | `com.ctrip.framework.apollo:apollo-client` | java | `1.7.1-hw-1.7.1.057` | `2.4.0` | 20 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 2926 | 2925 | `com.ctrip.framework.apollo:apollo-client` | java | `2.1.0` | `2.4.0` | 20 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.3.0`, `1.4.0`, `1.5.1`, `1.7.1-hw-1.7.1.057`, `2.1.0`。
- 升级候选边：`1.3.0`, `1.4.0`, `1.5.1`, `1.7.1-hw-1.7.1.057`, `2.1.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2926 2.1.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.4.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1326 1.3.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1327 1.4.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1328 1.5.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1329 1.7.1-hw-1.7.1.057 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.4.0` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
