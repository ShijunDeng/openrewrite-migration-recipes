# org.eclipse.jetty:jetty-xml 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-eclipse-jetty-jetty-xml` |
| Maven artifactId | `migration-spec-java-maven-org-eclipse-jetty-jetty-xml` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.eclipse.jetty:jetty-xml` |
| Catalog canonical identity | `org.eclipse.jetty:jetty-xml`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `12.0.34` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 783 | 782 | `org.eclipse.jetty:jetty-xml` | java | `9.4.39.v20210325` | `12.0.34` | 1 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #783 9.4.39.v20210325 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `12.0.34` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
