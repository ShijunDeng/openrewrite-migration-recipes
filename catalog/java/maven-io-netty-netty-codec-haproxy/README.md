# io.netty:netty-codec-haproxy 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-netty-netty-codec-haproxy` |
| Maven artifactId | `migration-spec-java-maven-io-netty-netty-codec-haproxy` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.netty:netty-codec-haproxy` |
| Catalog canonical identity | `io.netty:netty-codec-haproxy`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.1.136.Final` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3673 | 3672 | `io.netty:netty-codec-haproxy` | java | `4.1.118.Final` | `4.1.136.Final` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3674 | 3673 | `io.netty:netty-codec-haproxy` | java | `4.1.125.Final` | `4.1.136.Final` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3675 | 3674 | `io.netty:netty-codec-haproxy` | java | `4.1.86.Final` | `4.1.136.Final` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3676 | 3675 | `io.netty:netty-codec-haproxy` | java | `4.1.94.Final` | `4.1.136.Final` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.1.118.Final`, `4.1.125.Final`, `4.1.86.Final`, `4.1.94.Final`。
- 升级候选边：`4.1.118.Final`, `4.1.125.Final`, `4.1.86.Final`, `4.1.94.Final`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3673 4.1.118.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3674 4.1.125.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3675 4.1.86.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3676 4.1.94.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.1.136.Final` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
