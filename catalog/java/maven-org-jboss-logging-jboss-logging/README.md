# org.jboss.logging:jboss-logging 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-jboss-logging-jboss-logging` |
| Maven artifactId | `migration-spec-java-maven-org-jboss-logging-jboss-logging` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.jboss.logging:jboss-logging` |
| Catalog canonical identity | `org.jboss.logging:jboss-logging`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.6.3.Final` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2745 | 2744 | `org.jboss.logging:jboss-logging` | java | `3.3.3.Final` | `3.6.3.Final` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2746 | 2745 | `org.jboss.logging:jboss-logging` | java | `3.4.2.Final` | `3.6.3.Final` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2747 | 2746 | `org.jboss.logging:jboss-logging` | java | `3.4.3.Final` | `3.6.3.Final` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2748 | 2747 | `org.jboss.logging:jboss-logging` | java | `3.5.3.Final` | `3.6.3.Final` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.3.3.Final`, `3.4.2.Final`, `3.4.3.Final`, `3.5.3.Final`。
- 升级候选边：`3.3.3.Final`, `3.4.2.Final`, `3.4.3.Final`, `3.5.3.Final`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2745 3.3.3.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2746 3.4.2.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2747 3.4.3.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2748 3.5.3.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.6.3.Final` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
