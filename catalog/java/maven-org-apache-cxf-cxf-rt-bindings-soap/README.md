# org.apache.cxf:cxf-rt-bindings-soap 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-cxf-cxf-rt-bindings-soap` |
| Maven artifactId | `migration-spec-java-maven-org-apache-cxf-cxf-rt-bindings-soap` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.cxf:cxf-rt-bindings-soap` |
| Catalog canonical identity | `org.apache.cxf:cxf-rt-bindings-soap`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.1.3` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2029 | 2028 | `org.apache.cxf:cxf-rt-bindings-soap` | java | `3.5.6` | `4.1.3` | 1 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #2029 3.5.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.1.3` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
