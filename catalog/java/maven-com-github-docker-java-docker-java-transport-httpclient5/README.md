# com.github.docker-java:docker-java-transport-httpclient5 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-docker-java-docker-java-transport-httpclient5` |
| Maven artifactId | `migration-spec-java-maven-com-github-docker-java-docker-java-transport-httpclient5` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.docker-java:docker-java-transport-httpclient5` |
| Catalog canonical identity | `com.github.docker-java:docker-java-transport-httpclient5`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.3.4` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4755 | 4754 | `com.github.docker-java:docker-java-transport-httpclient5` | java | `3.3.2` | `3.3.4` | 1 | B3_Minor联动 | 低 | upgrade-candidate/mark | httpcomponents联动组patch直升 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.3.2`。
- 升级候选边：`3.3.2`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 依赖族对齐 / BOM / 平台 | Excel #4755 3.3.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.3.4` | httpcomponents联动组patch直升 | `UNVERIFIED` | 建立联动成员图并迁移 BOM、platform、parent 或共享 property 的真实 owner；禁止只改一个叶子依赖造成二进制或运行时漂移。 |
