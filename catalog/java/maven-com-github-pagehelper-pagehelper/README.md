# com.github.pagehelper:pagehelper 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-pagehelper-pagehelper` |
| Maven artifactId | `migration-spec-java-maven-com-github-pagehelper-pagehelper` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.pagehelper:pagehelper` |
| Catalog canonical identity | `com.github.pagehelper:pagehelper`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `6.1.1` |
| Excel 迁移边 | 4 |
| 涉及微服务数 | 最大可见值 `94`；不同版本行不累加 |
| 分桶 | `B4_Major单包` |
| 难度 | `中` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1008 | 1007 | `com.github.pagehelper:pagehelper` | java | `5.1.11` | `6.1.1` | 94 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1009 | 1008 | `com.github.pagehelper:pagehelper` | java | `5.3.1` | `6.1.1` | 94 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1010 | 1009 | `com.github.pagehelper:pagehelper` | java | `5.3.2` | `6.1.1` | 94 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1011 | 1010 | `com.github.pagehelper:pagehelper` | java | `5.3.3` | `6.1.1` | 94 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`5.1.11`, `5.3.1`, `5.3.2`, `5.3.3`。
- 升级候选边：`5.1.11`, `5.3.1`, `5.3.2`, `5.3.3`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1008 5.1.11 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1009 5.3.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1010 5.3.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1011 5.3.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.1.1` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
