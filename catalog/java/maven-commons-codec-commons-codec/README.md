# commons-codec:commons-codec 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-commons-codec-commons-codec` |
| Maven artifactId | `migration-spec-java-maven-commons-codec-commons-codec` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `commons-codec:commons-codec` |
| Catalog canonical identity | `commons-codec:commons-codec`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `1.22.0` |
| Excel 迁移边 | 5 |
| 涉及微服务数 | 最大可见值 `305`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2284 | 2283 | `commons-codec:commons-codec` | java | `1.11` | `1.22.0` | 305 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2285 | 2284 | `commons-codec:commons-codec` | java | `1.13` | `1.22.0` | 305 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2286 | 2285 | `commons-codec:commons-codec` | java | `1.14` | `1.22.0` | 305 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2287 | 2286 | `commons-codec:commons-codec` | java | `1.15` | `1.22.0` | 305 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2288 | 2287 | `commons-codec:commons-codec` | java | `1.16.0` | `1.22.0` | 305 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.11`, `1.13`, `1.14`, `1.15`, `1.16.0`。
- 升级候选边：`1.11`, `1.13`, `1.14`, `1.15`, `1.16.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2284 1.11 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2285 1.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2286 1.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2287 1.15 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2288 1.16.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.22.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
