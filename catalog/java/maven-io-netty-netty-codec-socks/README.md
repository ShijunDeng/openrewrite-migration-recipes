# io.netty:netty-codec-socks 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-io-netty-netty-codec-socks` |
| Maven artifactId | `migration-spec-java-maven-io-netty-netty-codec-socks` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `io.netty:netty-codec-socks` |
| Catalog canonical identity | `io.netty:netty-codec-socks`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.1.136.Final` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4430 | 4429 | `io.netty:netty-codec-socks` | java | `4.1.49.Final` | `4.1.136.Final` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4430 4.1.49.Final [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.1.136.Final` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
