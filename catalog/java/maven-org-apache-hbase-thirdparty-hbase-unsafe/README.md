# org.apache.hbase.thirdparty:hbase-unsafe 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-hbase-thirdparty-hbase-unsafe` |
| Maven artifactId | `migration-spec-java-maven-org-apache-hbase-thirdparty-hbase-unsafe` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.hbase.thirdparty:hbase-unsafe` |
| Catalog canonical identity | `org.apache.hbase.thirdparty:hbase-unsafe`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `4.1.12` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4359 | 4358 | `org.apache.hbase.thirdparty:hbase-unsafe` | java | `4.1.4` | `4.1.12` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4359 4.1.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.1.12` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
