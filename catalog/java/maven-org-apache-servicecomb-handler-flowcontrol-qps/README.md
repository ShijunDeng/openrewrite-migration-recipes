# org.apache.servicecomb:handler-flowcontrol-qps 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-apache-servicecomb-handler-flowcontrol-qps` |
| Maven artifactId | `migration-spec-java-maven-org-apache-servicecomb-handler-flowcontrol-qps` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.apache.servicecomb:handler-flowcontrol-qps` |
| Catalog canonical identity | `org.apache.servicecomb:handler-flowcontrol-qps`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.2.5` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1645 | 1644 | `org.apache.servicecomb:handler-flowcontrol-qps` | java | `2.1.5` | `3.2.5` | 7 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1646 | 1645 | `org.apache.servicecomb:handler-flowcontrol-qps` | java | `2.6.0` | `3.2.5` | 7 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.1.5`, `2.6.0`。
- 升级候选边：`2.1.5`, `2.6.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1645 2.1.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1646 2.6.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.2.5` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
