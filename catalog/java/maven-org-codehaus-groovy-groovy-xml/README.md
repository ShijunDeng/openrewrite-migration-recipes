# org.codehaus.groovy:groovy-xml 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-codehaus-groovy-groovy-xml` |
| Maven artifactId | `migration-spec-java-maven-org-codehaus-groovy-groovy-xml` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.codehaus.groovy:groovy-xml` |
| Catalog canonical identity | `org.codehaus.groovy:groovy-xml`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `3.0.21` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4059 | 4058 | `org.codehaus.groovy:groovy-xml` | java | `3.0.14` | `3.0.21` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 4060 | 4059 | `org.codehaus.groovy:groovy-xml` | java | `3.0.17` | `3.0.21` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.0.14`, `3.0.17`。
- 升级候选边：`3.0.14`, `3.0.17`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4059 3.0.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #4060 3.0.17 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.0.21` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
