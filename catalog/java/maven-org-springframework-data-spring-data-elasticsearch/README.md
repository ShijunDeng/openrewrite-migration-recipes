# org.springframework.data:spring-data-elasticsearch 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-springframework-data-spring-data-elasticsearch` |
| Maven artifactId | `migration-spec-java-maven-org-springframework-data-spring-data-elasticsearch` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.springframework.data:spring-data-elasticsearch` |
| Catalog canonical identity | `org.springframework.data:spring-data-elasticsearch`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `6.0.5` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 285 | 284 | `org.springframework.data:spring-data-elasticsearch` | java | `4.2.12` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 286 | 285 | `org.springframework.data:spring-data-elasticsearch` | java | `4.2.4` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 287 | 286 | `org.springframework.data:spring-data-elasticsearch` | java | `4.2.8` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 288 | 287 | `org.springframework.data:spring-data-elasticsearch` | java | `4.4.12` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 289 | 288 | `org.springframework.data:spring-data-elasticsearch` | java | `4.4.14` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 290 | 289 | `org.springframework.data:spring-data-elasticsearch` | java | `4.4.8` | `6.0.5` | 23 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`4.2.12`, `4.2.4`, `4.2.8`, `4.4.12`, `4.4.14`, `4.4.8`。
- 升级候选边：`4.2.12`, `4.2.4`, `4.2.8`, `4.4.12`, `4.4.14`, `4.4.8`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #285 4.2.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #286 4.2.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #287 4.2.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #288 4.4.12 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #289 4.4.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #290 4.4.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.0.5` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
