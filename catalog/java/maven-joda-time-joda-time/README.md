...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-joda-time-joda-time` |
| Maven artifactId | `migration-spec-java-maven-joda-time-joda-time` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `joda-time:joda-time` |
| Catalog canonical identity | `joda-time:joda-time`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `2.14.0` |
| Excel 迁移边 | 7 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2725 | 2724 | `joda-time:joda-time` | java | `2.10.13` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2726 | 2725 | `joda-time:joda-time` | java | `2.10.14` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2727 | 2726 | `joda-time:joda-time` | java | `2.10.3` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2728 | 2727 | `joda-time:joda-time` | java | `2.10.4` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2729 | 2728 | `joda-time:joda-time` | java | `2.12.2` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2730 | 2729 | `joda-time:joda-time` | java | `2.12.4` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2731 | 2730 | `joda-time:joda-time` | java | `2.12.5` | `2.14.0` | 28 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.10.13`, `2.10.14`, `2.10.3`, `2.10.4`, `2.12.2`, `2.12.4`, `2.12.5`。
- 升级候选边：`2.10.13`, `2.10.14`, `2.10.3`, `2.10.4`, `2.12.2`, `2.12.4`, `2.12.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2725 2.10.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2726 2.10.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2727 2.10.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2728 2.10.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2729 2.12.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2730 2.12.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2731 2.12.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.14.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
