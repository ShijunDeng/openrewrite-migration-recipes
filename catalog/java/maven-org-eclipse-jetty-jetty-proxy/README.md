# org.eclipse.jetty:jetty-proxy 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-org-eclipse-jetty-jetty-proxy` |
| Maven artifactId | `migration-spec-java-maven-org-eclipse-jetty-jetty-proxy` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `org.eclipse.jetty:jetty-proxy` |
| Catalog canonical identity | `org.eclipse.jetty:jetty-proxy`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `12.1.8` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 354 | 353 | `org.eclipse.jetty:jetty-proxy` | java | `9.4.39.v20210325` | `12.1.8` | 16 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 355 | 354 | `org.eclipse.jetty:jetty-proxy` | java | `9.4.45.v20220203` | `12.1.8` | 16 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`9.4.39.v20210325`, `9.4.45.v20220203`。
- 升级候选边：`9.4.39.v20210325`, `9.4.45.v20220203`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
