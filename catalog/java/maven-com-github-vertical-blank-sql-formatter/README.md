# com.github.vertical-blank:sql-formatter 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/java/maven-com-github-vertical-blank-sql-formatter` |
| Maven artifactId | `migration-spec-java-maven-com-github-vertical-blank-sql-formatter` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `com.github.vertical-blank:sql-formatter` |
| Catalog canonical identity | `com.github.vertical-blank:sql-formatter`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `java` |
| Excel 原始语言 | `java` |
| 目标版本 | `15.6.5` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 414 | 413 | `com.github.vertical-blank:sql-formatter` | java | `12.0.6` | `15.6.5` | 11 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 415 | 414 | `com.github.vertical-blank:sql-formatter` | java | `12.2.0` | `15.6.5` | 11 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 416 | 415 | `com.github.vertical-blank:sql-formatter` | java | `2.0.4` | `15.6.5` | 11 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 417 | 416 | `com.github.vertical-blank:sql-formatter` | java | `3.1.0` | `15.6.5` | 11 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`12.0.6`, `12.2.0`, `2.0.4`, `3.1.0`。
- 升级候选边：`12.0.6`, `12.2.0`, `2.0.4`, `3.1.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #414 12.0.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #415 12.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #416 2.0.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #417 3.1.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `15.6.5` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
