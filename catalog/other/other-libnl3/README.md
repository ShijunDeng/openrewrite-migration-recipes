...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-libnl3` |
| Maven artifactId | `migration-spec-other-other-libnl3` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `libnl3` |
| Catalog canonical identity | `libnl3`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `3.5.0-1_ky_x86_64` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4364 | 4363 | `libnl3` | other | `3.2.8eulerosv_x86_64` | `3.5.0-1_ky_x86_64` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 4365 | 4364 | `libnl3` | other | `3.4.0euleros_aarch64` | `3.5.0-1_ky_x86_64` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 4366 | 4365 | `libnl3` | other | `3.5.0euleros_aarch64` | `3.5.0-1_ky_x86_64` | 1 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 4367 | 4366 | `libnl3` | other | `3.5.0euleros_x86_64` | `3.5.0-1_ky_x86_64` | 1 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.2.8eulerosv_x86_64`, `3.4.0euleros_aarch64`, `3.5.0euleros_aarch64`, `3.5.0euleros_x86_64`。
- 升级候选边：`3.2.8eulerosv_x86_64`, `3.4.0euleros_aarch64`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`3.5.0euleros_aarch64`, `3.5.0euleros_x86_64`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4366 3.5.0euleros_aarch64 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #4367 3.5.0euleros_x86_64 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `3.5.0-1_ky_x86_64` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4364 3.2.8eulerosv_x86_64 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #4365 3.4.0euleros_aarch64 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.5.0-1_ky_x86_64` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
