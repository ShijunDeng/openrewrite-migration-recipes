...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-geos` |
| Maven artifactId | `migration-spec-other-other-geos` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `geos` |
| Catalog canonical identity | `geos`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `3.10.2` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4611 | 4610 | `geos` | other | `3.10.2` | `3.10.2` | 1 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.10.2`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4611 3.10.2 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `3.10.2` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
