...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-openssl` |
| Maven artifactId | `migration-spec-other-other-openssl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `openssl` |
| Catalog canonical identity | `openssl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `1.1.1t` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3444 | 3443 | `openssl` | other | `1.1.1d` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3445 | 3444 | `openssl` | other | `1.1.1k` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3446 | 3445 | `openssl` | other | `1.1.1l` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3447 | 3446 | `openssl` | other | `1.1.1q` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3448 | 3447 | `openssl` | other | `1.1.1q-hw-1.0` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3449 | 3448 | `openssl` | other | `1.1.1u` | `1.1.1t` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.1.1d`, `1.1.1k`, `1.1.1l`, `1.1.1q`, `1.1.1q-hw-1.0`, `1.1.1u`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`1.1.1d`, `1.1.1k`, `1.1.1l`, `1.1.1q`, `1.1.1q-hw-1.0`, `1.1.1u`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3444 1.1.1d [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3445 1.1.1k [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3446 1.1.1l [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3447 1.1.1q [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3448 1.1.1q-hw-1.0 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3449 1.1.1u [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `1.1.1t` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
