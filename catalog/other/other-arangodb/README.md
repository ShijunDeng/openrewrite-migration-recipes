...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-arangodb` |
| Maven artifactId | `migration-spec-other-other-arangodb` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `arangodb` |
| Catalog canonical identity | `arangodb`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `v3.7.8` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4591 | 4590 | `arangodb` | other | `v3.11.0_aarch64` | `v3.7.8` | 1 | B2_Minor单包 | 低 | conflict/mark | 同大版本内minor升级，通常向后兼容 |
| 4592 | 4591 | `arangodb` | other | `v3.7.8` | `v3.7.8` | 1 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v3.11.0_aarch64`, `v3.7.8`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`v3.7.8`。
- 潜在降级冲突：`v3.11.0_aarch64`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4592 v3.7.8 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `v3.7.8` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4591 v3.11.0_aarch64 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `v3.7.8` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
