...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-mockgen` |
| Maven artifactId | `migration-spec-other-other-mockgen` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `mockgen` |
| Catalog canonical identity | `mockgen`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `v1.6.0.aarch64` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4349 | 4348 | `mockgen` | other | `v1.6.0` | `v1.6.0.aarch64` | 1 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4349 v1.6.0 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `v1.6.0.aarch64` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
