...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-spacy-models` |
| Maven artifactId | `migration-spec-other-other-spacy-models` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `spacy-models` |
| Catalog canonical identity | `spacy-models`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `en_core_web_sm-3.5.0` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4730 | 4729 | `spacy-models` | other | `en_core_web_sm-3.5.0` | `en_core_web_sm-3.5.0` | 1 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`en_core_web_sm-3.5.0`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`en_core_web_sm-3.5.0`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4730 en_core_web_sm-3.5.0 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `en_core_web_sm-3.5.0` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
