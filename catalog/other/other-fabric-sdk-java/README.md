...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-fabric-sdk-java` |
| Maven artifactId | `migration-spec-other-other-fabric-sdk-java` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `fabric-sdk-java` |
| Catalog canonical identity | `fabric-sdk-java`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `v2.2.15-hw-1.0.0` |
| Excel 迁移边 | 2 |
| 涉及微服务数 | 最大可见值 `2`；不同版本行不累加 |
| 分桶 | `B0_无需升级`, `B4_Major单包` |
| 难度 | `中`, `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1932 | 1931 | `fabric-sdk-java` | other | `1.0.1` | `v2.2.15-hw-1.0.0` | 2 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 4213 | 4212 | `fabric-sdk-java` | other | `v2.2.15-hw-1.0.0` | `v2.2.15-hw-1.0.0` | 2 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.0.1`, `v2.2.15-hw-1.0.0`。
- 升级候选边：`1.0.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`v2.2.15-hw-1.0.0`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4213 v2.2.15-hw-1.0.0 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `v2.2.15-hw-1.0.0` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1932 1.0.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v2.2.15-hw-1.0.0` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
