...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-dotnet-sdk` |
| Maven artifactId | `migration-spec-other-other-dotnet-sdk` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `dotnet-sdk` |
| Catalog canonical identity | `dotnet-sdk`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `3.1.410` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 838 | 837 | `dotnet-sdk` | other | `5.0.400` | `3.1.410` | 1 | B6_Multi-major单包 | 高 | conflict/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 4666 | 4665 | `dotnet-sdk` | other | `3.1.410` | `3.1.410` | 1 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.1.410`, `5.0.400`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4666 3.1.410 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `3.1.410` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #838 5.0.400 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `3.1.410` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
