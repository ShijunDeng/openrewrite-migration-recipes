...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/c-c-openssl-openssl` |
| Maven artifactId | `migration-spec-other-c-c-openssl-openssl` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `openssl-openssl` |
| Catalog canonical identity | `openssl-openssl`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `c/c++` |
| 目标版本 | `1.1.1u-hw-cloudrtc-lll-1.0.4` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 343 | 342 | `openssl-openssl` | c/c++ | `3.0.9` | `1.1.1u-hw-cloudrtc-lll-1.0.4` | 17 | B6_Multi-major单包 | 高 | conflict/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 3041 | 3040 | `openssl-openssl` | c/c++ | `1.1.1u-hw-live-1.0` | `1.1.1u-hw-cloudrtc-lll-1.0.4` | 17 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.1.1u-hw-live-1.0`, `3.0.9`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`3.0.9`。
- 截断、聚合或无法可靠比较：`1.1.1u-hw-live-1.0`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3041 1.1.1u-hw-live-1.0 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `1.1.1u-hw-cloudrtc-lll-1.0.4` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #343 3.0.9 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `1.1.1u-hw-cloudrtc-lll-1.0.4` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
