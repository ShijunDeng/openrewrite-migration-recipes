...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/other/other-protobuf` |
| Maven artifactId | `migration-spec-other-other-protobuf` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `protobuf` |
| Catalog canonical identity | `protobuf`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `other` |
| Excel 原始语言 | `other` |
| 目标版本 | `0.15.0` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 613 | 612 | `protobuf` | other | `3.19.3` | `0.15.0` | 4 | B8_语义不兼容 | 高 | conflict/mark | protobuf跨多个大版本(3.19.3->0.15.0)，proto2/proto3差异 |
| 614 | 613 | `protobuf` | other | `3.20.3` | `0.15.0` | 4 | B8_语义不兼容 | 高 | conflict/mark | protobuf跨多个大版本(3.20.3->0.15.0)，proto2/proto3差异 |
| 615 | 614 | `protobuf` | other | `3.21.12` | `0.15.0` | 4 | B8_语义不兼容 | 高 | conflict/mark | protobuf跨多个大版本(3.21.12->0.15.0)，proto2/proto3差异 |
| 616 | 615 | `protobuf` | other | `3.5.1` | `0.15.0` | 4 | B8_语义不兼容 | 高 | conflict/mark | protobuf跨多个大版本(3.5.1->0.15.0)，proto2/proto3差异 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.19.3`, `3.20.3`, `3.21.12`, `3.5.1`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`3.19.3`, `3.20.3`, `3.21.12`, `3.5.1`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 行为契约 / 语义 / 真实用例 | Excel #613 3.19.3 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `0.15.0` | protobuf跨多个大版本(3.19.3->0.15.0)，proto2/proto3差异 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-002 | 行为契约 / 语义 / 真实用例 | Excel #614 3.20.3 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `0.15.0` | protobuf跨多个大版本(3.20.3->0.15.0)，proto2/proto3差异 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-003 | 行为契约 / 语义 / 真实用例 | Excel #615 3.21.12 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `0.15.0` | protobuf跨多个大版本(3.21.12->0.15.0)，proto2/proto3差异 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-004 | 行为契约 / 语义 / 真实用例 | Excel #616 3.5.1 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `0.15.0` | protobuf跨多个大版本(3.5.1->0.15.0)，proto2/proto3差异 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
