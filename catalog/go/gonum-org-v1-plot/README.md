...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/gonum-org-v1-plot` |
| Maven artifactId | `migration-spec-go-gonum-org-v1-plot` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `gonum.org/v1/plot` |
| Catalog canonical identity | `gonum.org/v1/plot`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.16.0` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4006 | 4005 | `gonum.org/v1/plot` | go | `v0.9.0` | `v0.16.0` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4006 v0.9.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v0.16.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
