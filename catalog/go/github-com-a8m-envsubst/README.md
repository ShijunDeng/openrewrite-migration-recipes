...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-a8m-envsubst` |
| Maven artifactId | `migration-spec-go-github-com-a8m-envsubst` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/a8m/envsubst` |
| Catalog canonical identity | `github.com/a8m/envsubst`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.4.3` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4597 | 4596 | `github.com/a8m/envsubst` | go | `v1.4.2` | `v1.4.3` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4597 v1.4.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.4.3` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
