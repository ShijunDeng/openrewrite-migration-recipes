...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-coredns-caddy` |
| Maven artifactId | `migration-spec-go-github-com-coredns-caddy` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/coredns/caddy` |
| Catalog canonical identity | `github.com/coredns/caddy`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.1.2-0.20241029205200-8de985351a98` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4573 | 4572 | `github.com/coredns/caddy` | go | `v1.1.1` | `v1.1.2-0.20241029205200-8de985351a98` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4573 v1.1.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.1.2-0.20241029205200-8de985351a98` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
