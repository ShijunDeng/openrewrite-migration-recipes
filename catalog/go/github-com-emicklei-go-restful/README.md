# github.com/emicklei/go-restful 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-emicklei-go-restful` |
| Maven artifactId | `migration-spec-go-github-com-emicklei-go-restful` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/emicklei/go-restful` |
| Catalog canonical identity | `github.com/emicklei/go-restful`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v3.11.0` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1568 | 1567 | `github.com/emicklei/go-restful` | go | `v2.15.0+incompatible` | `v3.11.0` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1569 | 1568 | `github.com/emicklei/go-restful` | go | `v2.16.0+incompatible` | `v3.11.0` | 9 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v2.15.0+incompatible`, `v2.16.0+incompatible`。
- 升级候选边：`v2.15.0+incompatible`, `v2.16.0+incompatible`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
