# github.com/hyperledger/fabric-chaincode-go 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-hyperledger-fabric-chaincode-go` |
| Maven artifactId | `migration-spec-go-github-com-hyperledger-fabric-chaincode-go` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/hyperledger/fabric-chaincode-go` |
| Catalog canonical identity | `github.com/hyperledger/fabric-chaincode-go`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.0.0-20230731094759-d626e9ab09b9` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3969 | 3968 | `github.com/hyperledger/fabric-chaincode-go` | go | `v0.0.0-20230228194215-b84622ba6a7a` | `v0.0.0-20230731094759-d626e9ab09b9` | 3 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.0.0-20230228194215-b84622ba6a7a`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`v0.0.0-20230228194215-b84622ba6a7a`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3969 v0.0.0-20230228194215-b84622ba6a7a [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `v0.0.0-20230731094759-d626e9ab09b9` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
