# github.com/agiledragon/gomonkey/v2 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-agiledragon-gomonkey-v2` |
| Maven artifactId | `migration-spec-go-github-com-agiledragon-gomonkey-v2` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/agiledragon/gomonkey/v2` |
| Catalog canonical identity | `github.com/agiledragon/gomonkey/v2`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v2.13.0` |
| Excel 迁移边 | 7 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2510 | 2509 | `github.com/agiledragon/gomonkey/v2` | go | `v2.10.0` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2511 | 2510 | `github.com/agiledragon/gomonkey/v2` | go | `v2.10.1` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2512 | 2511 | `github.com/agiledragon/gomonkey/v2` | go | `v2.2.0` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2513 | 2512 | `github.com/agiledragon/gomonkey/v2` | go | `v2.3.1` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2514 | 2513 | `github.com/agiledragon/gomonkey/v2` | go | `v2.7.0` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2515 | 2514 | `github.com/agiledragon/gomonkey/v2` | go | `v2.8.0` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2516 | 2515 | `github.com/agiledragon/gomonkey/v2` | go | `v2.9.0` | `v2.13.0` | 52 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v2.10.0`, `v2.10.1`, `v2.2.0`, `v2.3.1`, `v2.7.0`, `v2.8.0`, `v2.9.0`。
- 升级候选边：`v2.10.0`, `v2.10.1`, `v2.2.0`, `v2.3.1`, `v2.7.0`, `v2.8.0`, `v2.9.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2510 v2.10.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2511 v2.10.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2512 v2.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2513 v2.3.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2514 v2.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2515 v2.8.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2516 v2.9.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v2.13.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
