# github.com/stretchr/testify 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-stretchr-testify` |
| Maven artifactId | `migration-spec-go-github-com-stretchr-testify` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/stretchr/testify` |
| Catalog canonical identity | `github.com/stretchr/testify`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.11.1` |
| Excel 迁移边 | 7 |
| 涉及微服务数 | 最大可见值 `88`；不同版本行不累加 |
| 分桶 | `B2_Minor单包` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2383 | 2382 | `github.com/stretchr/testify` | go | `v1.7.0` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2384 | 2383 | `github.com/stretchr/testify` | go | `v1.7.2` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2385 | 2384 | `github.com/stretchr/testify` | go | `v1.8.0` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2386 | 2385 | `github.com/stretchr/testify` | go | `v1.8.1` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2387 | 2386 | `github.com/stretchr/testify` | go | `v1.8.2` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2388 | 2387 | `github.com/stretchr/testify` | go | `v1.8.3` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2389 | 2388 | `github.com/stretchr/testify` | go | `v1.8.4` | `v1.11.1` | 88 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.7.0`, `v1.7.2`, `v1.8.0`, `v1.8.1`, `v1.8.2`, `v1.8.3`, `v1.8.4`。
- 升级候选边：`v1.7.0`, `v1.7.2`, `v1.8.0`, `v1.8.1`, `v1.8.2`, `v1.8.3`, `v1.8.4`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2383 v1.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2384 v1.7.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2385 v1.8.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2386 v1.8.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2387 v1.8.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2388 v1.8.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2389 v1.8.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.11.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
