...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-onsi-gomega` |
| Maven artifactId | `migration-spec-go-github-com-onsi-gomega` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/onsi/gomega` |
| Catalog canonical identity | `github.com/onsi/gomega`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.39.1` |
| Excel 迁移边 | 8 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3429 | 3428 | `github.com/onsi/gomega` | go | `v1.14.0` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3430 | 3429 | `github.com/onsi/gomega` | go | `v1.20.0` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3431 | 3430 | `github.com/onsi/gomega` | go | `v1.20.1` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3432 | 3431 | `github.com/onsi/gomega` | go | `v1.21.1` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3433 | 3432 | `github.com/onsi/gomega` | go | `v1.24.1` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3434 | 3433 | `github.com/onsi/gomega` | go | `v1.25.0` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3435 | 3434 | `github.com/onsi/gomega` | go | `v1.27.4` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3436 | 3435 | `github.com/onsi/gomega` | go | `v1.27.5` | `v1.39.1` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.14.0`, `v1.20.0`, `v1.20.1`, `v1.21.1`, `v1.24.1`, `v1.25.0`, `v1.27.4`, `v1.27.5`。
- 升级候选边：`v1.14.0`, `v1.20.0`, `v1.20.1`, `v1.21.1`, `v1.24.1`, `v1.25.0`, `v1.27.4`, `v1.27.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3429 v1.14.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3430 v1.20.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3431 v1.20.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3432 v1.21.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3433 v1.24.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3434 v1.25.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3435 v1.27.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3436 v1.27.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.39.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
