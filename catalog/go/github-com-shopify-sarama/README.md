# github.com/Shopify/sarama 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-shopify-sarama` |
| Maven artifactId | `migration-spec-go-github-com-shopify-sarama` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/Shopify/sarama` |
| Catalog canonical identity | `github.com/Shopify/sarama`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.46.3` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2865 | 2864 | `github.com/Shopify/sarama` | go | `v1.28.0` | `v1.46.3` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2866 | 2865 | `github.com/Shopify/sarama` | go | `v1.36.0` | `v1.46.3` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2867 | 2866 | `github.com/Shopify/sarama` | go | `v1.37.2` | `v1.46.3` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2868 | 2867 | `github.com/Shopify/sarama` | go | `v1.38.1` | `v1.46.3` | 22 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.28.0`, `v1.36.0`, `v1.37.2`, `v1.38.1`。
- 升级候选边：`v1.28.0`, `v1.36.0`, `v1.37.2`, `v1.38.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2865 v1.28.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2866 v1.36.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2867 v1.37.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2868 v1.38.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.46.3` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
