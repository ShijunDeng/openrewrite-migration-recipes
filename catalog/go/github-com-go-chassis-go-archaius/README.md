# github.com/go-chassis/go-archaius 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-go-chassis-go-archaius` |
| Maven artifactId | `migration-spec-go-github-com-go-chassis-go-archaius` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/go-chassis/go-archaius` |
| Catalog canonical identity | `github.com/go-chassis/go-archaius`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.5.7-0.20250403063843-3dfdac2673a9` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3729 | 3728 | `github.com/go-chassis/go-archaius` | go | `v1.5.1` | `v1.5.7-0.20250403063843-3dfdac2673a9` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3730 | 3729 | `github.com/go-chassis/go-archaius` | go | `v1.5.4` | `v1.5.7-0.20250403063843-3dfdac2673a9` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3731 | 3730 | `github.com/go-chassis/go-archaius` | go | `v1.5.6` | `v1.5.7-0.20250403063843-3dfdac2673a9` | 5 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.5.1`, `v1.5.4`, `v1.5.6`。
- 升级候选边：`v1.5.1`, `v1.5.4`, `v1.5.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3729 v1.5.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3730 v1.5.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3731 v1.5.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.5.7-0.20250403063843-3dfdac2673a9` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
