...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/golang-org-x-time` |
| Maven artifactId | `migration-spec-go-golang-org-x-time` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `golang.org/x/time` |
| Catalog canonical identity | `golang.org/x/time`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.15.0` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2676 | 2675 | `golang.org/x/time` | go | `v0.0.0-20200630173020-3af7569d3a1e` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2677 | 2676 | `golang.org/x/time` | go | `v0.0.0-20210220033141-f8bda1e9f3ba` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2678 | 2677 | `golang.org/x/time` | go | `v0.0.0-20220210224613-90d013bbcef8` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2679 | 2678 | `golang.org/x/time` | go | `v0.0.0-20220722155302-e5dcc9cfc0b9` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2680 | 2679 | `golang.org/x/time` | go | `v0.2.0` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2681 | 2680 | `golang.org/x/time` | go | `v0.3.0` | `v0.15.0` | 31 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.0.0-20200630173020-3af7569d3a1e`, `v0.0.0-20210220033141-f8bda1e9f3ba`, `v0.0.0-20220210224613-90d013bbcef8`, `v0.0.0-20220722155302-e5dcc9cfc0b9`, `v0.2.0`, `v0.3.0`。
- 升级候选边：`v0.0.0-20200630173020-3af7569d3a1e`, `v0.0.0-20210220033141-f8bda1e9f3ba`, `v0.0.0-20220210224613-90d013bbcef8`, `v0.0.0-20220722155302-e5dcc9cfc0b9`, `v0.2.0`, `v0.3.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2676 v0.0.0-20200630173020-3af7569d3a1e [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2677 v0.0.0-20210220033141-f8bda1e9f3ba [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2678 v0.0.0-20220210224613-90d013bbcef8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2679 v0.0.0-20220722155302-e5dcc9cfc0b9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2680 v0.2.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2681 v0.3.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v0.15.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
