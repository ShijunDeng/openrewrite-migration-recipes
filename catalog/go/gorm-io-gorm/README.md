...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/gorm-io-gorm` |
| Maven artifactId | `migration-spec-go-gorm-io-gorm` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `gorm.io/gorm` |
| Catalog canonical identity | `gorm.io/gorm`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.30.0` |
| Excel 迁移边 | 6 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3118 | 3117 | `gorm.io/gorm` | go | `v1.23.1` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3119 | 3118 | `gorm.io/gorm` | go | `v1.23.4` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3120 | 3119 | `gorm.io/gorm` | go | `v1.24.2` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3121 | 3120 | `gorm.io/gorm` | go | `v1.25.1` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3122 | 3121 | `gorm.io/gorm` | go | `v1.25.2` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3123 | 3122 | `gorm.io/gorm` | go | `v1.25.2-0.20230530020048-26663ab9bf55` | `v1.30.0` | 15 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.23.1`, `v1.23.4`, `v1.24.2`, `v1.25.1`, `v1.25.2`, `v1.25.2-0.20230530020048-26663ab9bf55`。
- 升级候选边：`v1.23.1`, `v1.23.4`, `v1.24.2`, `v1.25.1`, `v1.25.2`, `v1.25.2-0.20230530020048-26663ab9bf55`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3118 v1.23.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3119 v1.23.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3120 v1.24.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3121 v1.25.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3122 v1.25.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3123 v1.25.2-0.20230530020048-26663ab9bf55 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.30.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
