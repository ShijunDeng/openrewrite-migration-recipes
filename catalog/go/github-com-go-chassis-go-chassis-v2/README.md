# github.com/go-chassis/go-chassis/v2 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-go-chassis-go-chassis-v2` |
| Maven artifactId | `migration-spec-go-github-com-go-chassis-go-chassis-v2` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/go-chassis/go-chassis/v2` |
| Catalog canonical identity | `github.com/go-chassis/go-chassis/v2`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v2.7.2-0.20240320130142-593a156e1025` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3899 | 3898 | `github.com/go-chassis/go-chassis/v2` | go | `v2.3.0` | `v2.7.2-0.20240320130142-593a156e1025` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3900 | 3899 | `github.com/go-chassis/go-chassis/v2` | go | `v2.7.0` | `v2.7.2-0.20240320130142-593a156e1025` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3901 | 3900 | `github.com/go-chassis/go-chassis/v2` | go | `v2.7.1` | `v2.7.2-0.20240320130142-593a156e1025` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v2.3.0`, `v2.7.0`, `v2.7.1`。
- 升级候选边：`v2.3.0`, `v2.7.0`, `v2.7.1`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3900 v2.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3901 v2.7.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v2.7.2-0.20240320130142-593a156e1025` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3899 v2.3.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v2.7.2-0.20240320130142-593a156e1025` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
