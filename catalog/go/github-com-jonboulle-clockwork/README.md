# github.com/jonboulle/clockwork 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-jonboulle-clockwork` |
| Maven artifactId | `migration-spec-go-github-com-jonboulle-clockwork` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/jonboulle/clockwork` |
| Catalog canonical identity | `github.com/jonboulle/clockwork`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.5.0` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3967 | 3966 | `github.com/jonboulle/clockwork` | go | `v0.2.2` | `v0.5.0` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3968 | 3967 | `github.com/jonboulle/clockwork` | go | `v0.4.0` | `v0.5.0` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.2.2`, `v0.4.0`。
- 升级候选边：`v0.2.2`, `v0.4.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3967 v0.2.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3968 v0.4.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v0.5.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
