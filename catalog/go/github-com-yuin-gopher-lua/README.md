# github.com/yuin/gopher-lua 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-yuin-gopher-lua` |
| Maven artifactId | `migration-spec-go-github-com-yuin-gopher-lua` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/yuin/gopher-lua` |
| Catalog canonical identity | `github.com/yuin/gopher-lua`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.1.1` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4155 | 4154 | `github.com/yuin/gopher-lua` | go | `v1.1.0` | `v1.1.1` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4155 v1.1.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.1.1` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
