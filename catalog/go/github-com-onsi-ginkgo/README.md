...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-onsi-ginkgo` |
| Maven artifactId | `migration-spec-go-github-com-onsi-ginkgo` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/onsi/ginkgo` |
| Catalog canonical identity | `github.com/onsi/ginkgo`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v2.6.0` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1703 | 1702 | `github.com/onsi/ginkgo` | go | `v1.16.4` | `v2.6.0` | 6 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1704 | 1703 | `github.com/onsi/ginkgo` | go | `v1.16.5` | `v2.6.0` | 6 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 1705 | 1704 | `github.com/onsi/ginkgo` | go | `v1.16.6-0.20211118180735-4e1925ba4c95` | `v2.6.0` | 6 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.16.4`, `v1.16.5`, `v1.16.6-0.20211118180735-4e1925ba4c95`。
- 升级候选边：`v1.16.4`, `v1.16.5`, `v1.16.6-0.20211118180735-4e1925ba4c95`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1703 v1.16.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1704 v1.16.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #1705 v1.16.6-0.20211118180735-4e1925ba4c95 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v2.6.0` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
