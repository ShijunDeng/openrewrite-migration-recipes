# github.com/huaweicloud/huaweicloud-sdk-go-obs 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-huaweicloud-huaweicloud-sdk-go-obs` |
| Maven artifactId | `migration-spec-go-github-com-huaweicloud-huaweicloud-sdk-go-obs` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` |
| Catalog canonical identity | `github.com/huaweicloud/huaweicloud-sdk-go-obs`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v3.25.4+incompatible` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3022 | 3021 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` | go | `v3.21.12+incompatible` | `v3.25.4+incompatible` | 17 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3023 | 3022 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` | go | `v3.21.8+incompatible` | `v3.25.4+incompatible` | 17 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3024 | 3023 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` | go | `v3.22.11+incompatible` | `v3.25.4+incompatible` | 17 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3025 | 3024 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` | go | `v3.23.3+incompatible` | `v3.25.4+incompatible` | 17 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3026 | 3025 | `github.com/huaweicloud/huaweicloud-sdk-go-obs` | go | `v3.23.4+incompatible` | `v3.25.4+incompatible` | 17 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v3.21.12+incompatible`, `v3.21.8+incompatible`, `v3.22.11+incompatible`, `v3.23.3+incompatible`, `v3.23.4+incompatible`。
- 升级候选边：`v3.21.12+incompatible`, `v3.21.8+incompatible`, `v3.22.11+incompatible`, `v3.23.3+incompatible`, `v3.23.4+incompatible`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3022 v3.21.12+incompatible [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3023 v3.21.8+incompatible [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3024 v3.22.11+incompatible [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3025 v3.23.3+incompatible [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3026 v3.23.4+incompatible [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v3.25.4+incompatible` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
