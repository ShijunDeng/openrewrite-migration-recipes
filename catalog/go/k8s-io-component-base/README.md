...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/k8s-io-component-base` |
| Maven artifactId | `migration-spec-go-k8s-io-component-base` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `k8s.io/component-base` |
| Catalog canonical identity | `k8s.io/component-base`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.34.1` |
| Excel 迁移边 | 5 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3641 | 3640 | `k8s.io/component-base` | go | `v0.19.16` | `v0.34.1` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3642 | 3641 | `k8s.io/component-base` | go | `v0.19.16-s1.0.5` | `v0.34.1` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3643 | 3642 | `k8s.io/component-base` | go | `v0.24.3` | `v0.34.1` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3644 | 3643 | `k8s.io/component-base` | go | `v0.26.3` | `v0.34.1` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3645 | 3644 | `k8s.io/component-base` | go | `v0.27.0` | `v0.34.1` | 6 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.19.16`, `v0.19.16-s1.0.5`, `v0.24.3`, `v0.26.3`, `v0.27.0`。
- 升级候选边：`v0.19.16`, `v0.19.16-s1.0.5`, `v0.24.3`, `v0.26.3`, `v0.27.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3641 v0.19.16 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3642 v0.19.16-s1.0.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3643 v0.24.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3644 v0.26.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3645 v0.27.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v0.34.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
