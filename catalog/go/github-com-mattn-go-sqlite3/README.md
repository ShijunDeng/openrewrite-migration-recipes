# github.com/mattn/go-sqlite3 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-mattn-go-sqlite3` |
| Maven artifactId | `migration-spec-go-github-com-mattn-go-sqlite3` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/mattn/go-sqlite3` |
| Catalog canonical identity | `github.com/mattn/go-sqlite3`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.14.32` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1330 | 1329 | `github.com/mattn/go-sqlite3` | go | `v2.0.6+incompatible` | `v1.14.32` | 20 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |
| 2943 | 2942 | `github.com/mattn/go-sqlite3` | go | `v1.14.16` | `v1.14.32` | 20 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2944 | 2943 | `github.com/mattn/go-sqlite3` | go | `v1.14.17` | `v1.14.32` | 20 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.14.16`, `v1.14.17`, `v2.0.6+incompatible`。
- 升级候选边：`v1.14.16`, `v1.14.17`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`v2.0.6+incompatible`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2943 v1.14.16 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2944 v1.14.17 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.14.32` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1330 v2.0.6+incompatible [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `v1.14.32` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
