# github.com/mitchellh/mapstructure 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-mitchellh-mapstructure` |
| Maven artifactId | `migration-spec-go-github-com-mitchellh-mapstructure` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/mitchellh/mapstructure` |
| Catalog canonical identity | `github.com/mitchellh/mapstructure`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.5.1-0.20231216201459-8508981c8b6c` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3438 | 3437 | `github.com/mitchellh/mapstructure` | go | `v1.4.3` | `v1.5.1-0.20231216201459-8508981c8b6c` | 8 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 3439 | 3438 | `github.com/mitchellh/mapstructure` | go | `v1.5.0` | `v1.5.1-0.20231216201459-8508981c8b6c` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3440 | 3439 | `github.com/mitchellh/mapstructure` | go | `v1.5.1-0.20220423185008-bf980b35cac4` | `v1.5.1-0.20231216201459-8508981c8b6c` | 8 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.4.3`, `v1.5.0`, `v1.5.1-0.20220423185008-bf980b35cac4`。
- 升级候选边：`v1.4.3`, `v1.5.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`v1.5.1-0.20220423185008-bf980b35cac4`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3440 v1.5.1-0.20220423185008-bf980b35cac4 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `v1.5.1-0.20231216201459-8508981c8b6c` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 补丁行为 / 安全 / 回归 | Excel #3439 v1.5.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.5.1-0.20231216201459-8508981c8b6c` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-003 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3438 v1.4.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.5.1-0.20231216201459-8508981c8b6c` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
