# github.com/segmentio/kafka-go 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-segmentio-kafka-go` |
| Maven artifactId | `migration-spec-go-github-com-segmentio-kafka-go` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/segmentio/kafka-go` |
| Catalog canonical identity | `github.com/segmentio/kafka-go`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.4.48` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3255 | 3254 | `github.com/segmentio/kafka-go` | go | `v0.4.38` | `v0.4.48` | 12 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3256 | 3255 | `github.com/segmentio/kafka-go` | go | `v0.4.40` | `v0.4.48` | 12 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.4.38`, `v0.4.40`。
- 升级候选边：`v0.4.38`, `v0.4.40`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3255 v0.4.38 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3256 v0.4.40 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v0.4.48` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
