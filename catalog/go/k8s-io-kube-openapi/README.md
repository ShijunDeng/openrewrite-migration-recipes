...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/k8s-io-kube-openapi` |
| Maven artifactId | `migration-spec-go-k8s-io-kube-openapi` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `k8s.io/kube-openapi` |
| Catalog canonical identity | `k8s.io/kube-openapi`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.0.0-20241105132330-32ad38e42d3f` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3647 | 3646 | `k8s.io/kube-openapi` | go | `v0.0.0-20221012153701-172d655c2280` | `v0.0.0-20241105132330-32ad38e42d3f` | 6 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3648 | 3647 | `k8s.io/kube-openapi` | go | `v0.0.0-20230501164219-8b0f38b5fd1f` | `v0.0.0-20241105132330-32ad38e42d3f` | 6 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |
| 3649 | 3648 | `k8s.io/kube-openapi` | go | `v0.0.0-20230717233707-2695361300d9` | `v0.0.0-20241105132330-32ad38e42d3f` | 6 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.0.0-20221012153701-172d655c2280`, `v0.0.0-20230501164219-8b0f38b5fd1f`, `v0.0.0-20230717233707-2695361300d9`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`v0.0.0-20221012153701-172d655c2280`, `v0.0.0-20230501164219-8b0f38b5fd1f`, `v0.0.0-20230717233707-2695361300d9`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3647 v0.0.0-20221012153701-172d655c2280 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3648 v0.0.0-20230501164219-8b0f38b5fd1f [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。]<br>Excel #3649 v0.0.0-20230717233707-2695361300d9 [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `v0.0.0-20241105132330-32ad38e42d3f` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
