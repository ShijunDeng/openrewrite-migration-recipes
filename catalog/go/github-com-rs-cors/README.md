...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-rs-cors` |
| Maven artifactId | `migration-spec-go-github-com-rs-cors` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/rs/cors` |
| Catalog canonical identity | `github.com/rs/cors`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.11.1` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4525 | 4524 | `github.com/rs/cors` | go | `v1.8.0` | `v1.11.1` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4525 v1.8.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.11.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
