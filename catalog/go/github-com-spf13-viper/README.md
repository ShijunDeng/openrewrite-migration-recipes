...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-spf13-viper` |
| Maven artifactId | `migration-spec-go-github-com-spf13-viper` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/spf13/viper` |
| Catalog canonical identity | `github.com/spf13/viper`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.21.0` |
| Excel 迁移边 | 7 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2878 | 2877 | `github.com/spf13/viper` | go | `v1.11.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2879 | 2878 | `github.com/spf13/viper` | go | `v1.13.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2880 | 2879 | `github.com/spf13/viper` | go | `v1.14.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2881 | 2880 | `github.com/spf13/viper` | go | `v1.15.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2882 | 2881 | `github.com/spf13/viper` | go | `v1.16.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2883 | 2882 | `github.com/spf13/viper` | go | `v1.8.1` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
| 2884 | 2883 | `github.com/spf13/viper` | go | `v1.9.0` | `v1.21.0` | 21 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.11.0`, `v1.13.0`, `v1.14.0`, `v1.15.0`, `v1.16.0`, `v1.8.1`, `v1.9.0`。
- 升级候选边：`v1.11.0`, `v1.13.0`, `v1.14.0`, `v1.15.0`, `v1.16.0`, `v1.8.1`, `v1.9.0`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #2878 v1.11.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2879 v1.13.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2880 v1.14.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2881 v1.15.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2882 v1.16.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2883 v1.8.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2884 v1.9.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.21.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
