...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/github-com-spf13-pflag` |
| Maven artifactId | `migration-spec-go-github-com-spf13-pflag` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `github.com/spf13/pflag` |
| Catalog canonical identity | `github.com/spf13/pflag`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v1.0.10` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2663 | 2662 | `github.com/spf13/pflag` | go | `v1.0.5` | `v1.0.10` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2664 | 2663 | `github.com/spf13/pflag` | go | `v1.0.6-0.20210604193023-d5e0c0615ace` | `v1.0.10` | 31 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v1.0.5`, `v1.0.6-0.20210604193023-d5e0c0615ace`。
- 升级候选边：`v1.0.5`, `v1.0.6-0.20210604193023-d5e0c0615ace`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2663 v1.0.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2664 v1.0.6-0.20210604193023-d5e0c0615ace [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `v1.0.10` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
