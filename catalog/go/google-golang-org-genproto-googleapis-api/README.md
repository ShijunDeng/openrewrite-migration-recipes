# google.golang.org/genproto/googleapis/api 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/go/google-golang-org-genproto-googleapis-api` |
| Maven artifactId | `migration-spec-go-google-golang-org-genproto-googleapis-api` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `google.golang.org/genproto/googleapis/api` |
| Catalog canonical identity | `google.golang.org/genproto/googleapis/api`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `go` |
| Excel 原始语言 | `go` |
| 目标版本 | `v0.0.0-20250707201910-8d1bb00bc6a7` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4487 | 4486 | `google.golang.org/genproto/googleapis/api` | go | `v0.0.0-20230530153820-e85fd2cbaebc` | `v0.0.0-20250707201910-8d1bb00bc6a7` | 1 | B0_无需升级 | 低 | unknown/mark | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`v0.0.0-20230530153820-e85fd2cbaebc`。
- 升级候选边：`NONE`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`v0.0.0-20230530153820-e85fd2cbaebc`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #4487 v0.0.0-20230530153820-e85fd2cbaebc [unknown/mark: 数字前缀相同但限定符或版本文字不同；未证明顺序前保持原样并 MARK。] → `v0.0.0-20250707201910-8d1bb00bc6a7` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
