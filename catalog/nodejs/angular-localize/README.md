...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/angular-localize` |
| Maven artifactId | `migration-spec-nodejs-angular-localize` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@angular/localize` |
| Catalog canonical identity | `@angular/localize`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `20.3.16` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 738 | 737 | `@angular/localize` | nodejs | `10.0.14` | `20.3.16` | 1 | B8_语义不兼容 | 高 | upgrade-candidate/mark | Angular跨多个大版本(10.0.14->20.3.16)，模板/装饰器/依赖注入大改 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`10.0.14`。
- 升级候选边：`10.0.14`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 行为契约 / 语义 / 真实用例 | Excel #738 10.0.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `20.3.16` | Angular跨多个大版本(10.0.14->20.3.16)，模板/装饰器/依赖注入大改 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
