...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/angular-compiler-cli` |
| Maven artifactId | `migration-spec-nodejs-angular-compiler-cli` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@angular/compiler-cli` |
| Catalog canonical identity | `@angular/compiler-cli`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `20.3.25` |
| Excel 迁移边 | 4 |
| 涉及微服务数 | 最大可见值 `5`；不同版本行不累加 |
| 分桶 | `B8_语义不兼容` |
| 难度 | `高` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 551 | 550 | `@angular/compiler-cli` | nodejs | `10.2.5` | `20.3.25` | 5 | B8_语义不兼容 | 高 | upgrade-candidate/mark | Angular跨多个大版本(10.2.5->20.3.25)，模板/装饰器/依赖注入大改 |
| 552 | 551 | `@angular/compiler-cli` | nodejs | `12.2.13` | `20.3.25` | 5 | B8_语义不兼容 | 高 | upgrade-candidate/mark | Angular跨多个大版本(12.2.13->20.3.25)，模板/装饰器/依赖注入大改 |
| 553 | 552 | `@angular/compiler-cli` | nodejs | `14.3.0` | `20.3.25` | 5 | B8_语义不兼容 | 高 | upgrade-candidate/mark | Angular跨多个大版本(14.3.0->20.3.25)，模板/装饰器/依赖注入大改 |
| 554 | 553 | `@angular/compiler-cli` | nodejs | `9.1.13` | `20.3.25` | 5 | B8_语义不兼容 | 高 | upgrade-candidate/mark | Angular跨多个大版本(9.1.13->20.3.25)，模板/装饰器/依赖注入大改 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`10.2.5`, `12.2.13`, `14.3.0`, `9.1.13`。
- 升级候选边：`10.2.5`, `12.2.13`, `14.3.0`, `9.1.13`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 行为契约 / 语义 / 真实用例 | Excel #551 10.2.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `20.3.25` | Angular跨多个大版本(10.2.5->20.3.25)，模板/装饰器/依赖注入大改 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-002 | 行为契约 / 语义 / 真实用例 | Excel #552 12.2.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `20.3.25` | Angular跨多个大版本(12.2.13->20.3.25)，模板/装饰器/依赖注入大改 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-003 | 行为契约 / 语义 / 真实用例 | Excel #553 14.3.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `20.3.25` | Angular跨多个大版本(14.3.0->20.3.25)，模板/装饰器/依赖注入大改 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
| C-004 | 行为契约 / 语义 / 真实用例 | Excel #554 9.1.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `20.3.25` | Angular跨多个大版本(9.1.13->20.3.25)，模板/装饰器/依赖注入大改 | `UNVERIFIED` | 必须固定官方源码或发布说明并建立行为契约；语义敏感项默认 MARK/MANUAL，只有上下文无歧义且可证明等价的修改才能 AUTO。 |
