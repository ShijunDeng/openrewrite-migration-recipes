...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/moment-timezone` |
| Maven artifactId | `migration-spec-nodejs-moment-timezone` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `moment-timezone` |
| Catalog canonical identity | `moment-timezone`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `0.5.48` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3631 | 3630 | `moment-timezone` | nodejs | `0.5.34` | `0.5.48` | 6 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3632 | 3631 | `moment-timezone` | nodejs | `0.5.41` | `0.5.48` | 6 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3633 | 3632 | `moment-timezone` | nodejs | `0.5.43` | `0.5.48` | 6 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`0.5.34`, `0.5.41`, `0.5.43`。
- 升级候选边：`0.5.34`, `0.5.41`, `0.5.43`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3631 0.5.34 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3632 0.5.41 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3633 0.5.43 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `0.5.48` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
