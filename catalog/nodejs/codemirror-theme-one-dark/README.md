# @codemirror/theme-one-dark 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/codemirror-theme-one-dark` |
| Maven artifactId | `migration-spec-nodejs-codemirror-theme-one-dark` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@codemirror/theme-one-dark` |
| Catalog canonical identity | `@codemirror/theme-one-dark`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `6.1.3` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4083 | 4082 | `@codemirror/theme-one-dark` | nodejs | `6.1.2` | `6.1.3` | 2 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4083 6.1.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.1.3` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
