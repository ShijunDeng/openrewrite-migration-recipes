...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/cross-spawn` |
| Maven artifactId | `migration-spec-nodejs-cross-spawn` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `cross-spawn` |
| Catalog canonical identity | `cross-spawn`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `6.0.6` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1888 | 1887 | `cross-spawn` | nodejs | `7.0.3` | `6.0.6` | 2 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`7.0.3`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1888 7.0.3 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `6.0.6` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
