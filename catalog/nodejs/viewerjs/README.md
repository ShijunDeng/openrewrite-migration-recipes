...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/viewerjs` |
| Maven artifactId | `migration-spec-nodejs-viewerjs` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `viewerjs` |
| Catalog canonical identity | `viewerjs`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `1.11.7` |
| Excel 迁移边 | 5 |
| 涉及微服务数 | 最大可见值 `3`；不同版本行不累加 |
| 分桶 | `B0_无需升级`, `B1_Patch直升` |
| 难度 | `低` |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3943 | 3942 | `viewerjs` | nodejs | `1.11.0` | `1.11.7` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3944 | 3943 | `viewerjs` | nodejs | `1.11.3` | `1.11.7` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3945 | 3944 | `viewerjs` | nodejs | `1.11.4` | `1.11.7` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3946 | 3945 | `viewerjs` | nodejs | `1.11.6` | `1.11.7` | 3 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 3947 | 3946 | `viewerjs` | nodejs | `1.11.7` | `1.11.7` | 3 | B0_无需升级 | 低 | same/noop | 版本相同或无法解析，可忽略 |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.11.0`, `1.11.3`, `1.11.4`, `1.11.6`, `1.11.7`。
- 升级候选边：`1.11.0`, `1.11.3`, `1.11.4`, `1.11.6`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`1.11.7`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 版本身份 / 所有权 | Excel #3947 1.11.7 [same/noop: 源与目标逐字相同；保持 NOOP，不把相同行伪装成升级。] → `1.11.7` | 版本相同或无法解析，可忽略 | `UNVERIFIED` | 先区分源目标确实相同与版本无法解析；前者保持 NOOP，后者在真实 owner 上 MARK，不得把“可忽略”当作已经证明兼容。 |
| C-002 | 补丁行为 / 安全 / 回归 | Excel #3943 1.11.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3944 1.11.3 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3945 1.11.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #3946 1.11.6 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.11.7` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
