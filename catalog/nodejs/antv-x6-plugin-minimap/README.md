...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/antv-x6-plugin-minimap` |
| Maven artifactId | `migration-spec-nodejs-antv-x6-plugin-minimap` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@antv/x6-plugin-minimap` |
| Catalog canonical identity | `@antv/x6-plugin-minimap`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `2.0.7` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4695 | 4694 | `@antv/x6-plugin-minimap` | nodejs | `2.0.2` | `2.0.7` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4695 2.0.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `2.0.7` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
