...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/fullcalendar-interaction` |
| Maven artifactId | `migration-spec-nodejs-fullcalendar-interaction` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@fullcalendar/interaction` |
| Catalog canonical identity | `@fullcalendar/interaction`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `6.1.17` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1787 | 1786 | `@fullcalendar/interaction` | nodejs | `5.7.0` | `6.1.17` | 4 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 3800 | 3799 | `@fullcalendar/interaction` | nodejs | `6.1.8` | `6.1.17` | 4 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`5.7.0`, `6.1.8`。
- 升级候选边：`5.7.0`, `6.1.8`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3800 6.1.8 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.1.17` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1787 5.7.0 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `6.1.17` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
