...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/cnchar` |
| Maven artifactId | `migration-spec-nodejs-cnchar` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `cnchar` |
| Catalog canonical identity | `cnchar`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `3.2.6` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 1224 | 1223 | `cnchar` | nodejs | `2.2.9` | `3.2.6` | 27 | B4_Major单包 | 中 | upgrade-candidate/mark | 跨1个大版本，需查changelog确认breaking API |
| 2784 | 2783 | `cnchar` | nodejs | `3.2.2` | `3.2.6` | 27 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2785 | 2784 | `cnchar` | nodejs | `3.2.4` | `3.2.6` | 27 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`2.2.9`, `3.2.2`, `3.2.4`。
- 升级候选边：`2.2.9`, `3.2.2`, `3.2.4`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2784 3.2.2 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2785 3.2.4 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.2.6` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1224 2.2.9 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `3.2.6` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
