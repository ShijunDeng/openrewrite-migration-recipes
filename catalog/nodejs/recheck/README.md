...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/recheck` |
| Maven artifactId | `migration-spec-nodejs-recheck` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `recheck` |
| Catalog canonical identity | `recheck`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `4.5.0` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 3903 | 3902 | `recheck` | nodejs | `4.4.5` | `4.5.0` | 3 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #3903 4.4.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `4.5.0` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
