...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/xss` |
| Maven artifactId | `migration-spec-nodejs-xss` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `xss` |
| Catalog canonical identity | `xss`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `1.0.15` |
| Excel 迁移边 | 4 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 2560 | 2559 | `xss` | nodejs | `1.0.10` | `1.0.15` | 44 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2561 | 2560 | `xss` | nodejs | `1.0.11` | `1.0.15` | 44 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2562 | 2561 | `xss` | nodejs | `1.0.13` | `1.0.15` | 44 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
| 2563 | 2562 | `xss` | nodejs | `1.0.14` | `1.0.15` | 44 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`1.0.10`, `1.0.11`, `1.0.13`, `1.0.14`。
- 升级候选边：`1.0.10`, `1.0.11`, `1.0.13`, `1.0.14`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #2560 1.0.10 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2561 1.0.11 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2562 1.0.13 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #2563 1.0.14 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.0.15` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
