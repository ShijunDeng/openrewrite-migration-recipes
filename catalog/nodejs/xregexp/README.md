...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/xregexp` |
| Maven artifactId | `migration-spec-nodejs-xregexp` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `xregexp` |
| Catalog canonical identity | `xregexp`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `5.1.2` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4799 | 4798 | `xregexp` | nodejs | `5.1.1` | `5.1.2` | 1 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #4799 5.1.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `5.1.2` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
