...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/vant` |
| Maven artifactId | `migration-spec-nodejs-vant` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `vant` |
| Catalog canonical identity | `vant`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `2.13.8` |
| Excel 迁移边 | 2 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 630 | 629 | `vant` | nodejs | `4.6.1` | `2.13.8` | 3 | B6_Multi-major单包 | 高 | conflict/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 1832 | 1831 | `vant` | nodejs | `3.6.12` | `2.13.8` | 3 | B4_Major单包 | 中 | conflict/mark | 跨1个大版本，需查changelog确认breaking API |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`3.6.12`, `4.6.1`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 公开 API / 配置 / 默认行为 / 运行时 | Excel #1832 3.6.12 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `2.13.8` | 跨1个大版本，需查changelog确认breaking API | `UNVERIFIED` | 建立跨主版本兼容矩阵；覆盖删除或重命名 API、配置键和默认值、运行时基线、模块系统、数据格式与回滚，AUTO 仅限已证明等价的确定性修改。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #630 4.6.1 [conflict/mark: 保守数字比较显示源版本高于目标；保持原文并标记目标版本冲突（禁止降级）。] → `2.13.8` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
