...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/patch-package` |
| Maven artifactId | `migration-spec-nodejs-patch-package` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `patch-package` |
| Catalog canonical identity | `patch-package`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `8.0.1` |
| Excel 迁移边 | 3 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 470 | 469 | `patch-package` | nodejs | `6.4.7` | `8.0.1` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 471 | 470 | `patch-package` | nodejs | `6.5.1` | `8.0.1` | 8 | B6_Multi-major单包 | 高 | upgrade-candidate/mark | 跨2+个大版本，breaking change概率极高，需API迁移 |
| 3437 | 3436 | `patch-package` | nodejs | `8.0.0-canary.5` | `8.0.1` | 8 | B1_Patch直升 | 低 | upgrade-candidate/mark | 仅patch变更，无breaking change |

## 升级方向与禁止降级

- 表格原始源版本记录（不是 AUTO 白名单）：`6.4.7`, `6.5.1`, `8.0.0-canary.5`。
- 升级候选边：`6.4.7`, `6.5.1`, `8.0.0-canary.5`；在 E-001～E-003 完成前仍保持 `MARK`。
- 相同版本 NOOP：`NONE`。
- 潜在降级冲突：`NONE`。
- 截断、聚合或无法可靠比较：`NONE`。
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 补丁行为 / 安全 / 回归 | Excel #3437 8.0.0-canary.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `8.0.1` | 仅patch变更，无breaking change | `UNVERIFIED` | 固定官方补丁说明和制品身份后才允许版本 AUTO；仍需验证安全修复、默认行为、序列化与协议回归。 |
| C-002 | 多主版本 API / 数据 / 协议 / 工具链 | Excel #470 6.4.7 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。]<br>Excel #471 6.5.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `8.0.1` | 跨2+个大版本，breaking change概率极高，需API迁移 | `UNVERIFIED` | 按每个中间主版本逐跳建立证据和回归门禁，不把多跳升级伪装成一次兼容升级；需要分阶段处理源码、配置、数据、协议、运行时与回滚。 |
