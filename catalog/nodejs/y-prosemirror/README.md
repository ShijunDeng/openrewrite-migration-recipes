...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/y-prosemirror` |
| Maven artifactId | `migration-spec-nodejs-y-prosemirror` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `y-prosemirror` |
| Catalog canonical identity | `y-prosemirror`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `1.3.7` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4312 | 4311 | `y-prosemirror` | nodejs | `1.2.1` | `1.3.7` | 2 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4312 1.2.1 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `1.3.7` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
