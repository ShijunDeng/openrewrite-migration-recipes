# @babel/plugin-transform-flow-strip-types 升级规格
...
| 字段 | 值 |
| --- | --- |
| Catalog 路径 | `catalog/nodejs/babel-plugin-transform-flow-strip-types` |
| Maven artifactId | `migration-spec-nodejs-babel-plugin-transform-flow-strip-types` |
| groupId | `com.huawei.clouds.openrewrite` |
| 规范表格标识 | `@babel/plugin-transform-flow-strip-types` |
| Catalog canonical identity | `@babel/plugin-transform-flow-strip-types`（`UNVERIFIED`，只用于避免目录碰撞） |
| 归一语言类 | `nodejs` |
| Excel 原始语言 | `nodejs` |
| 目标版本 | `7.27.1` |
| Excel 迁移边 | 1 |
...
| ---: | ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |
| 4601 | 4600 | `@babel/plugin-transform-flow-strip-types` | nodejs | `7.22.5` | `7.27.1` | 1 | B2_Minor单包 | 低 | upgrade-candidate/mark | 同大版本内minor升级，通常向后兼容 |
...
| --- | --- | --- | --- | --- | --- |
| C-001 | 弃用 / 默认值 / 配置 / 运行时 | Excel #4601 7.22.5 [upgrade-candidate/mark: 表格方向看似升级，但制品身份和官方兼容证据未固定；当前仅作为候选边。] → `7.27.1` | 同大版本内minor升级，通常向后兼容 | `UNVERIFIED` | 同一主版本不等于绝对兼容；核查弃用删除、默认值、运行时基线、传递依赖和配置，只自动处理有固定上游证据的一一对应修改。 |
