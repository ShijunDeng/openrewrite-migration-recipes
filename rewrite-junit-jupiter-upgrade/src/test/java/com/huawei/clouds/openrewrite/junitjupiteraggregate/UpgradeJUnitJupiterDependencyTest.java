...
class UpgradeJUnitJupiterDependencyTest implements RewriteTest {
    @Override
    public void defaults(RecipeSpec spec) { spec.recipe(new UpgradeSelectedJUnitJupiterDependency()); }
    ...
    @Test
    void whitelistExactlyMatchesRows1626To1628() {
        assertEquals(Set.of("5.9.1", "5.8.2", "5.9.3"),
                UpgradeSelectedJUnitJupiterDependency.SOURCE_VERSIONS);
    }
    ...
    void rootPropertyVisibleInProfileAndProfileOverrideWins() {
        rewriteRun(
                xml(project("<properties><r>5.9.1</r></properties><profiles><profile><id>it</id><dependencies>" + dep("${r}") + "</dependencies></profile></profiles>"),
    ...
    void upgradesRootAndProfileDependencyManagement() {
        rewriteRun(xml(
                project("<dependencyManagement><dependencies>" + dep("5.9.1") + "</dependencies></dependencyManagement>" +
    ...
    static Stream<Arguments> ambiguousProperties() {
        return Stream.of(
                Arguments.of("unused", project("<properties><r>5.9.1</r></properties>")),
                Arguments.of("plugin", project("<properties><r>5.9.1</r></properties><dependencies>" + dep("${r}") + "</dependencies><build><finalName>${r}</finalName></build>")),
                ...
                Arguments.of("root use not changed by unused profile override", project("<properties><r>2.0</r></properties><dependencies>" + dep("${r}") + "</dependencies><profiles><profile><id>x</id><properties><r>5.9.1</r></properties></profile></profiles>")),
                Arguments.of("profile shared", project("<profiles><profile><id>x</id><properties><r>5.9.1</r></properties><dependencies>" + dep("${r}") + rawDep("x", "y", "${r}", "") + "</dependencies></profile></profiles>")),
                Arguments.of("embedded expression", project("<properties><r>5.9.1</r></properties><dependencies>" + dep("${r}-custom") + "</dependencies>"))
        );
    }
    ...
    void upgradesRootGradleGroovyAndKotlin(String version) {
        rewriteRun(
                buildGradle("dependencies { implementation 'org.junit.jupiter:junit-jupiter:" + version + "' }",
                        "dependencies { implementation 'org.junit.jupiter:junit-jupiter:6.0.1' }"),
                buildGradleKts("dependencies { compileOnly(\"org.junit.jupiter:junit-jupiter:" + version + "\") }",
                        "dependencies { compileOnly(\"org.junit.jupiter:junit-jupiter:6.0.1\") }"));
    }

    @Test
    void upgradesGroovyMapNotations() {
        rewriteRun(
                buildGradle("dependencies { implementation group: 'org.junit.jupiter', name: 'junit-jupiter', version: '5.9.1', transitive: false }",
                        "dependencies { implementation group: 'org.junit.jupiter', name: 'junit-jupiter', version: '6.0.1', transitive: false }"),
                buildGradle("dependencies { implementation([group: 'org.junit.jupiter', name: 'junit-jupiter', version: '5.9.3']) }",
                        "dependencies { implementation([group: 'org.junit.jupiter', name: 'junit-jupiter', version: '6.0.1']) }"));
    }
    ...
    static Stream<Arguments> nestedGradle() {
        String d = "'org.junit.jupiter:junit-jupiter:5.9.1'";
        return Stream.of(
    ...
    static Stream<Arguments> wrongCoordinates() {
        return Stream.of(
                Arguments.of("wrong group", rawDep("org.apache.commons", "commons-codec", "5.9.1", "")),
                Arguments.of("Commons IO", rawDep("commons-io", "commons-io", "5.9.1", "")),
                ...
                Arguments.of("lookalike group", rawDep("org.junit.jupiterx", "commons-codec", "5.9.1", "")),
                Arguments.of("classified", rawDep("org.junit.jupiter", "junit-jupiter", "5.9.1", "<classifier>sources</classifier>")),
                Arguments.of("zip", rawDep("org.junit.jupiter", "junit-jupiter", "5.8.2", "<type>zip</type>")),
                Arguments.of("tests", rawDep("org.junit.jupiter", "junit-jupiter", "5.9.3", "<classifier>tests</classifier>"))
        );
    }
    ...
    void generatedCacheParentsAreNoop(String parent) {
        rewriteRun(xml(pom("5.9.1"), s -> s.path(parent + "/pom.xml")));
    }

    @Test
    void variantsAndVersionlessAreNoop() {
        rewriteRun(
                xml(project("<dependencies><dependency><groupId>org.junit.jupiter</groupId><artifactId>junit-jupiter</artifactId></dependency></dependencies>"), s -> s.path("pom.xml")),
                buildGradle("dependencies { implementation 'org.junit.jupiter:junit-jupiter:5.9.1:tests' }"),
                buildGradle("dependencies { implementation 'org.junit.jupiter:junit-jupiter:5.8.2@zip' }"));
    }

    @Test
    void installLeafProcessedAndTwoCyclesIdempotent() {
        rewriteRun(spec -> spec.cycles(2).expectedCyclesThatMakeChanges(1),
                xml(pom("5.9.1"), pom("6.0.1"), s -> s.path("pom.xml")),
                buildGradle("dependencies { implementation 'org.junit.jupiter:junit-jupiter:5.9.3' }",
                        "dependencies { implementation 'org.junit.jupiter:junit-jupiter:6.0.1' }", s -> s.path("install.gradle")));
    }

    @Test
    void recommendedStartsWithPublicUpgrade() {
        Environment environment = Environment.builder().scanRuntimeClasspath("com.huawei.clouds.openrewrite.junitjupiteraggregate").build();
        var migrate = environment.activateRecipes("com.huawei.clouds.openrewrite.junitjupiteraggregate.MigrateJUnitJupiterTo6_0_1");
        assertEquals("com.huawei.clouds.openrewrite.junitjupiteraggregate.UpgradeJUnitJupiterTo6_0_1",
    ...
    static String dep(String version) { return rawDep("org.junit.jupiter", "junit-jupiter", version, ""); }
    static String dep(String version, String extra) { return rawDep("org.junit.jupiter", "junit-jupiter", version, extra); }
    static String depWithoutVersion() { return "<dependency><groupId>org.junit.jupiter</groupId><artifactId>junit-jupiter</artifactId></dependency>"; }
