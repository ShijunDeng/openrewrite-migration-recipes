...
final class ElasticsearchUpgradeSupport {
    static final String GROUP = "org.testcontainers";
    static final String ARTIFACT = "elasticsearch";
    static final String SOURCE = "1.17.6";
    static final String TARGET = "1.21.4";
    static final String SERVER_GROUP = "org.elasticsearch";
    static final String SERVER_ARTIFACT = "elasticsearch";
    static final Set<String> SERVER_WORKBOOK_VERSIONS = Set.of("7.9.3", "7.10.2");
    ...
    private ElasticsearchUpgradeSupport() {
    }
    ...
    static boolean targetConflict(String version) {
        ...
        int majorOrder = major.compareTo(BigInteger.ONE);
        int minorOrder = minor.compareTo(BigInteger.valueOf(21));
        return majorOrder > 0 ||
               majorOrder == 0 && minorOrder > 0 ||
               majorOrder == 0 && minorOrder == 0 && patch.compareTo(BigInteger.valueOf(4)) > 0;
    }

    static boolean isTestcontainersDependency(Cursor cursor, Xml.Tag tag) {
        return isProjectDependency(cursor, tag) && coordinate(tag, GROUP, ARTIFACT);
    }

    static boolean isElasticsearchServerDependency(Cursor cursor, Xml.Tag tag) {
        return isProjectDependency(cursor, tag) && coordinate(tag, SERVER_GROUP, SERVER_ARTIFACT);
    }

    private static boolean coordinate(Xml.Tag tag, String group, String artifact) {
        return group.equals(tag.getChildValue("groupId").map(String::trim).orElse(null)) &&
