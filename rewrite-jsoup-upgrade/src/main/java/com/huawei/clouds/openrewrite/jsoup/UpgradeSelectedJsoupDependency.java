...
/** Upgrades only the exact jsoup source versions selected by the workbook. */
public final class UpgradeSelectedJsoupDependency extends Recipe {
    private static final String PREFIX = JsoupSupport.GROUP + ":" + JsoupSupport.ARTIFACT + ":";
    private static final Pattern PROPERTY_REFERENCE = Pattern.compile("\\$\\{([^}]+)}");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected jsoup dependencies to 1.21.1";
    }

    @Override
    public String getDescription() {
        return "Upgrades only exact org.jsoup:jsoup versions 1.14.2, 1.14.3, 1.15.3, 1.15.4, and 1.16.1 in owned standard Maven or Gradle declarations.";
    }
    ...
    private static G.CompilationUnit migrateGroovy(G.CompilationUnit source, ExecutionContext ctx) {
        return (G.CompilationUnit) new GroovyIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext executionContext) {
                boolean dependency = JsoupSupport.isGradleDependencyInvocation(getCursor(), method);
                J.MethodInvocation visited = super.visitMethodInvocation(method, executionContext);
                if (!dependency) return visited;
                if (coordinate(JsoupSupport.mapValue(visited, "group"), JsoupSupport.mapValue(visited, "name")) &&
                    JsoupSupport.SOURCES.contains(JsoupSupport.mapValue(visited, "version")) && !JsoupSupport.hasVariant(visited)) {
                    return visited.withArguments(visited.getArguments().stream().map(argument ->
                            argument instanceof G.MapEntry entry && "version".equals(JsoupSupport.mapKey(entry)) &&
                            entry.getValue() instanceof J.Literal literal ?
                                    entry.withValue(JsoupSupport.replaceLiteral(literal, JsoupSupport.TARGET)) : argument).toList());
                }
                return visited.withArguments(visited.getArguments().stream().map(argument ->
            ...
            public J.Literal visitLiteral(J.Literal literal, ExecutionContext executionContext) {
                boolean dependency = JsoupSupport.isDirectDependencyLiteral(getCursor());
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Set<PropertyOwner> safe = targetReferences.keySet().stream()
                .filter(owner -> values.get(owner) != null && JsoupSupport.SOURCES.contains(values.get(owner)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                Xml.Tag visited = super.visitTag(tag, executionContext);
                if (JsoupSupport.isMavenPropertyDefinition(getCursor(), visited) && safe.contains(propertyOwner(getCursor(), visited.getName())) &&
                    visited.getValue().map(String::trim).filter(JsoupSupport.SOURCES::contains).isPresent()) {
                    return visited.withValue(JsoupSupport.TARGET);
                }
                if (JsoupSupport.isJsoupDependency(getCursor(), visited) && JsoupSupport.standardJar(visited) &&
                    visited.getChildValue("version").map(String::trim).filter(JsoupSupport.SOURCES::contains).isPresent()) {
                    return visited.withChildValue("version", JsoupSupport.TARGET);
                }
                return visited;
            }
        }.visitNonNull(document, ctx);
    ...
    private static boolean targetVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               JsoupSupport.isJsoupDependency(dependencyCursor, dependency) && JsoupSupport.standardJar(dependency);
    }
    ...
    private static boolean coordinate(String group, String artifact) {
        return JsoupSupport.GROUP.equals(group) && JsoupSupport.ARTIFACT.equals(artifact);
    }

    private static G.MapLiteral upgradeMap(G.MapLiteral map) {
        if (!coordinate(JsoupSupport.mapValue(map, "group"), JsoupSupport.mapValue(map, "name")) ||
            !JsoupSupport.SOURCES.contains(JsoupSupport.mapValue(map, "version")) || JsoupSupport.hasVariant(map)) return map;
        return map.withElements(map.getElements().stream().map(entry ->
                "version".equals(JsoupSupport.mapKey(entry)) && entry.getValue() instanceof J.Literal literal ?
                        entry.withValue(JsoupSupport.replaceLiteral(literal, JsoupSupport.TARGET)) : entry).toList());
    }
    ...
    private static J.Literal upgradeCoordinate(J.Literal literal) {
        ...
        return JsoupSupport.SOURCES.contains(version) ? JsoupSupport.replaceLiteral(literal, PREFIX + JsoupSupport.TARGET) : literal;
    }
